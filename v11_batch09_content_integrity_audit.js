...
for(const p of ps){
 ...
 if(p.questionStage!=='BATCH09_HUMAN_REWRITE_R1')bad(p.id,'question_stage',String(p.questionStage));
 ...
 for(const [name,qs] of [['A',p.questions],['B',p.questionSetB]]){
   if(!Array.isArray(qs)||qs.length!==5){bad(p.id,`${name}_count`,String(qs&&qs.length));continue;} totalQuestions+=qs.length;
   qs.forEach((q,i)=>{if(!q||!String(q.prompt||'').trim())bad(p.id,`${name}_prompt`,String(i+1));if(!String(q.answer||'').trim())bad(p.id,`${name}_answer`,String(i+1));if(!String(q.evidence||'').trim())bad(p.id,`${name}_evidence`,String(i+1));if(!String(q.evidenceJp||'').trim())bad(p.id,`${name}_evidenceJp`,String(i+1));if(!String(q.reason||'').trim())bad(p.id,`${name}_reason`,String(i+1));
     if(/第\d+段階|第\d+文の内容を表す空所|本文の第\d+文/.test(String(q.prompt||'')))bad(p.id,`${name}_mechanical_scaffold`,`${i+1}:${q.prompt}`);
     ...
     } else {const n=wc(q.answer);if(n<20||n>30)bad(p.id,'free_write_length',`${n} words`);if(!q.scoring||!Array.isArray(q.scoring.conditions)||q.scoring.conditions.length<2)bad(p.id,'free_write_scoring','missing explicit conditions');}
   });
 }
 ...
 if(p.level==='YAMAGUCHI_EXAM'&&![...(p.questions||[]),...(p.questionSetB||[])].some(q=>q.questionType==='MATERIAL_LINK'))bad(p.id,'material_question','Yamaguchi item missing MATERIAL_LINK');
 ...
 rows.push({id:p.id,grade:p.grade,wordCount:actual,semantic:!!p.semanticRewrite,questionStage:p.questionStage,a:(p.questions||[]).length,b:(p.questionSetB||[]).length,types:types.size});
}
const out={generatedAt:new Date().toISOString(),passages:ps.length,semanticComplete:ps.filter(p=>p.semanticRewrite&&p.semanticRewrite.humanRead).length,totalQuestions,failures:fail.length,finalPass:ps.length===50&&totalQuestions===500&&fail.length===0,fail,rows};fs.writeFileSync('V11_BATCH09_CONTENT_INTEGRITY_REPORT.json',JSON.stringify(out,null,2)+'\n');fs.writeFileSync('V11_BATCH09_CONTENT_INTEGRITY_STATUS.txt',[`passages=${ps.length}/50`,`semantic_complete=${out.semanticComplete}/50`,`questions=${totalQuestions}/500`,`failures=${fail.length}`,`final=${out.finalPass?'PASS':'FAIL'}`].join('\n')+'\n');console.log(fs.readFileSync('V11_BATCH09_CONTENT_INTEGRITY_STATUS.txt','utf8'));if(!out.finalPass)process.exitCode=1;
