...
for(const p of ps){
 ...
 for(const [english,japanese] of Object.entries(gloss)){
  if(body.has(english)&&!have.has(english)){
   p.notes.push({english,japanese,kind:'unlearned_local_required',source:'v11 Batch02 story-specific local vocabulary chronology repair'});
   have.add(english);added++;
  }
 }
}
window.V11_BATCH02_REQUIRED_NOTES_REPAIR_STATE={version:'20260828-pass2',count:ps.length,added,registered:false};
})();