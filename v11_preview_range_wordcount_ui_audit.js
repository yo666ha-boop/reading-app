...
async function auditEngine(browserType,name){
  ...
  for(const s of samples){
    ...
    need(x.wordFilter,`${name} ${s.section}: Word数 filter missing`,failures);need(x.opts.length>1,`${name} ${s.section}: passage candidates=${x.opts.length}`,failures);need(labels.some(t=>/^基本｜/.test(t)),`${name} ${s.section}: no 基本 label`,failures);need(labels.some(t=>/^追加\d+｜/.test(t)),`${name} ${s.section}: no 追加 label`,failures);need(labels.every(t=>/\|\d+ words\|/.test(t.replace(/｜/g,'|'))),`${name} ${s.section}: word count missing from option label`,failures);need(sections.length>=2,`${name} ${s.section}: learned-range sections=${sections.join(',')}`,failures);need(/候補 \d+題（基本\d+・追加\d+）/.test(x.master),`${name} ${s.section}: candidate summary missing`,failures);
    const extra=x.opts.find(o=>/^追加\d+｜/.test(o.label));
    if(extra){
      await page.selectOption('#v11PassageVariant',extra.value);await page.waitForTimeout(120);x=await inspect(page);need(x.id===extra.value,`${name} ${s.section}: extra selection mismatch`,failures);need(x.meta.includes('追加本文')&&x.meta.includes(x.words+' words'),`${name} ${s.section}: selected source/words meta missing`,failures);need(norm(x.passage).includes(norm(x.fullTranslation)),`${name} ${s.section}: translation not synced`,failures);need(x.slashRows[0]&&norm(x.slash).includes(norm(x.slashRows[0].en))&&norm(x.slash).includes(norm(x.slashRows[0].jp)),`${name} ${s.section}: slash not synced`,failures);need(x.qA.length===5&&norm(x.questions).includes(qtext(x.qA[0]&&x.qA[0].prompt)),`${name} ${s.section}: A questions not synced`,failures);need(x.qA[0]&&norm(x.answers).includes(norm(x.qA[0].evidence))&&norm(x.answers).includes(norm(x.qA[0].evidenceJp)),`${name} ${s.section}: A evidence not synced`,failures);
    ...
    results.push({...s,optionCount:x.opts.length,sections,selectedId:x.id,words:x.words});
  }
