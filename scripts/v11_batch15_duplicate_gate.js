...
function ctxFor(batchNo,captured){const c={console:{log(){},warn(){},error(){}},setTimeout(){return 0},clearTimeout(){},Event:function(){},CustomEvent:function(){},fetch(){return Promise.reject(new Error('fetch disabled in static extractor'))}};c.window=c;c.globalThis=c;c.V11_EXTRA_PASSAGES={};c.document={createElement(){return {dataset:{},style:{},set src(v){this._src=v},get src(){return this._src},set async(v){},onload:null,onerror:null}},head:{appendChild(){}},body:{appendChild(){}},querySelector(){return null},getElementById(){return null}};c.dispatchEvent=()=>{};c.V11_APPLY_EASY_SUPPORT_NOTES=()=>{};c.V11_REGISTER_PASSAGES=list=>{const arr=list||[];c.V11_EXTRA_PASSAGES[`B${batchNo}`]=arr;for(const p of arr){const b=text(p);if(p&&p.id&&words(b).length>=20)captured.set(String(p.id),{id:String(p.id),body:b,source:`runtime-b${String(batchNo).padStart(2,'0')}`});}return{extraPassages:batchNo*50,lastAdded:arr.length}};vm.createContext(c);return c}
...
function registeredV11FromRuntimeSources(){const captured=new Map();{
 const c=ctxFor(1,captured);runFile(c,'v11_batch01_passages_001_050.js');for(const f of ['v11_batch01_uniqueness_repair.js','v11_batch01_grammar_repair.js'])if(fs.existsSync(f))try{runFile(c,f)}catch(e){}
 }
 ...
 return captured}
function baselineV10(){const out=new Map();for(const f of all.filter(f=>/^v10_data_.*\.js$/i.test(path.basename(f)))){const c={window:{},console:{log(){},warn(){},error(){}}};c.globalThis=c;vm.createContext(c);try{vm.runInContext(fs.readFileSync(f,'utf8'),c,{timeout:3000})}catch{continue}visit(c.window,o=>{const b=text(o);if(words(b).length<10)return;const n=norm(b);if(n&&!out.has(n))out.set(n,{id:`V10:${path.basename(f)}:${out.size+1}`,body:b,source:f});});}return out}
