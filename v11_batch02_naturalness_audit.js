...
try{
 ...
 const oldSafeMarker=ps.filter(p=>String(p.auditNote||'').includes('same-unit sentence bank')&&!p.semanticRewrite).map(p=>p.id);
 const semanticPending=ps.filter(p=>!p.semanticRewrite).map(p=>p.id);
 const rewritten=ps.filter(p=>/^PASS[123]_GRADE[123]_20260828$/.test(String(p.semanticRewrite||''))).map(p=>p.id);
 ...
 const structuralPass=ps.length===50&&rewritten.length===50&&pairs.length===0&&oldSafeMarker.length===0&&arcOnly.length===0;
 const out={generatedAt:new Date().toISOString(),passages:ps.length,semanticRewriteComplete:rewritten.length,semanticRewritePendingCount:semanticPending.length,highSharedPairs:pairs.length,worstPairs:pairs.sort((a,b)=>b.score-a.score).slice(0,30),sameUnitRepairMarkerCount:oldSafeMarker.length,highCommonSentenceRatio:arcOnly.length,structuralPass,finalPass:structuralPass&&semanticPending.length===0};
