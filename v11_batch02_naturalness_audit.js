...
try{
 ...
 run(s,'v11_batch02_passages_draft.js');run(s,'v11_batch02_unit_safe_repair.js');run(s,'v11_batch02_required_notes_repair.js');
 ...
 const safeMarker=ps.filter(p=>String(p.auditNote||'').includes('same-unit sentence bank')).map(p=>p.id);
 ...
 const out={generatedAt:new Date().toISOString(),passages:ps.length,highSharedPairs:pairs.length,worstPairs:pairs.sort((a,b)=>b.score-a.score).slice(0,30),sameUnitRepairMarkerCount:safeMarker.length,highCommonSentenceRatio:arcOnly.length,examples:arcOnly.slice(0,20),finalPass:ps.length===50&&pairs.length===0&&safeMarker.length===0&&arcOnly.length===0};
