...
function loadMany(files,key){const c={window:{}};vm.createContext(c);for(const f of files)vm.runInContext(fs.readFileSync(f,'utf8'),c,{filename:f});return c.window[key]||{}}
const data=loadMany(['v10_data_sunshine_g2_program1.js','v10_data_sunshine_g2_program2.js'],'V10_PASSAGES_G2_SS');
const meta1=loadMany(['v10_interaction_metadata_sun_g2_p1.js'],'V10_INTERACTION_META_G2');
const meta2=loadMany(['v10_interaction_metadata_sun_g2_p2.js'],'V10_INTERACTION_META_G2_P2');
const meta=Object.assign({},meta1,meta2);
const expected=['PROGRAM 1-1','PROGRAM 1-2','PROGRAM 1-3','PROGRAM 2-1','PROGRAM 2-2','PROGRAM 2-3'];const errors=[];let bq=0;
...
for(const [section,m] of Object.entries(data)){
 const tag=`SS2/${section}`;if(m.grade!=='2'||m.textbook!=='サンシャイン'||m.section!==section)errors.push(`${tag}: identity mismatch`);
 for(const k of ['id','title','fullTranslation','auditNote'])if(!filled(m[k]))errors.push(`${tag}: ${k} empty`);
 if(!Array.isArray(m.sentences)||m.sentences.length<10)errors.push(`${tag}: fewer than 10 sentences`);if(!Array.isArray(m.slashRows)||m.slashRows.length!==m.sentences.length)errors.push(`${tag}: slash count mismatch`);if(Array.isArray(m.slashRows)&&m.slashRows.some(r=>!r||!filled(r.en)||!filled(r.jp)))errors.push(`${tag}: empty slash row`);
...
if(Object.keys(meta).length!==expected.length)errors.push(`G2 interaction expected ${expected.length}, got ${Object.keys(meta).length}`);
console.log(`G2 AUDIT sunshine=${Object.keys(data).length}/${expected.length} alternate_questions=${bq}`);if(errors.length){console.error(`G2 AUDIT FAIL ${errors.length}`);for(const e of errors)console.error(`- ${e}`);process.exit(1)}console.log('G2 AUDIT PASS: Sunshine Program 1 through 2 passages, slash rows, A/B questions, evidence links, genres, and release flags are consistent.');
