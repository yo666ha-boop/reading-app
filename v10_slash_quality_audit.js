...
(async()=>{
 ...
 for(const[g,b,d]of sets)for(const[sec,p]of Object.entries(d)){
  ...
  r.forEach((row,i)=>{
   ...
   if(wc(s[i])<=6&&ep.length>1)errs.push(`${tag2}: forced slash in short basic sentence: ${en}`);
 ...
 console.log('SLASH QUALITY PASS: meaning chunks preserve every English sentence, short basic sentences are not force-split, core grammar is not divided, and EN/JP chunk counts agree.');dom.window.close();
