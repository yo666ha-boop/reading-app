...
try{
  ...
  const failures=[];let total=0,reused=0,arc=0,uniquePromptForms=new Set();
  for(const p of ps){
  ...
  const out={generatedAt:new Date().toISOString(),passages:ps.length,totalQuestions:total,reusedAuditedContentQuestions:reused,storyArcExtractiveQuestions:arc,uniquePromptForms:uniquePromptForms.size,failures,finalPass:ps.length===50&&total===500&&reused>=400&&failures.length===0};
