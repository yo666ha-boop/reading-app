...
async function engine(type){
  ...
  try{
    ...
    ok(ids.length===50,type+' b14 count');
    ok(new Set(ids).size===50,type+' b14 unique ids');
    let overflow=0;
    for(const id of ids){
      const x=await c.page.evaluate(id=>window.V11_BATCH14_PASSAGES.find(p=>p.id===id),id);
      await c.page.selectOption('#textbook',x.textbook);
      await c.page.selectOption('#grade',String(x.grade));
      const majors=await c.page.locator('#major option').evaluateAll(os=>os.map(o=>o.value));
      let found=false;
      for(const m of majors){
        await c.page.selectOption('#major',m);
        const secs=await c.page.locator('#section option').evaluateAll(os=>os.map(o=>o.value));
        if(secs.includes(x.section)){found=true;break}
      }
      ok(found,id+' section');
      await c.page.selectOption('#section',x.section);
      const idx=await c.page.evaluate(id=>{const p=window.V11_BATCH14_PASSAGES.find(p=>p.id===id),a=window.V11_EXTRA_PASSAGES[[p.textbook,String(p.grade),p.section].join('|')]||[];return a.findIndex(q=>q.id===id)+1},id);
      ok(idx>0,id+' variant');
      await c.page.selectOption('#v11PassageVariant',String(idx));
      await c.page.evaluate(()=>window.render());
      ok((await c.page.locator('#questions .q').count())===5,id+' A');
      const b=c.page.locator('#altSetBtn');
      if(await b.count()){await b.click();ok((await c.page.locator('#questions .q').count())===5,id+' B')}
      if(await c.page.evaluate(()=>document.documentElement.scrollWidth>document.documentElement.clientWidth+2))overflow++;
    }
    ok(!overflow,type+' overflow');
    ok(!c.errors.length,type+' errors '+c.errors.join('|'));
...
(async()=>{
  const pc=await engine('chromium'),iphone=await engine('webkit');
  const out={generatedAt:new Date().toISOString(),registered:true,total:868,extra:700,batch14:50,pc,iphone,finalPass:true};
  fs.writeFileSync('V11_BATCH14_PERSISTENT_RUNTIME_AUDIT.json',JSON.stringify(out,null,2)+'\n');
  console.log(JSON.stringify(out,null,2));
  console.log('V11_BATCH14_PERSISTENT_868_PASS');
