...
function syncStage(stage,x){if(stage==='R7')return syncR7(x);if(stage==='R8')return syncR8(x);if(stage==='R11')return syncR11(x);return x;}
const r4Passages=Object.entries(r4.questionRewrites||{}).map(([id,v])=>({id,questions:v.A,questionSetB:v.B}));add('R4',r4Passages);
for(const [stage,path] of stages){let x=read(path);if(x.registered!==false||x.officialTotal!==718)fail(stage+' state');x=syncStage(stage,x);add(stage,x.passages||[])}
