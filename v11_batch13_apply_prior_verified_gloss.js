...
function loadGloss(){
  ...
  const b13=require('./v11_batch13_manual_gloss_r1.js');
  return {...(sandbox.window.V11_BATCH10_PRIOR_VERIFIED_GLOSS||{}),...(sandbox.window.V11_BATCH10_MANUAL_GLOSS||{}),...(sandbox.window.V11_BATCH11_MANUAL_GLOSS||{}),...b12,...b13};
}
...
module.exports=function apply(candidate){const inv=JSON.parse(fs.readFileSync('V11_BATCH13_VOCAB_INVENTORY.json','utf8')),gloss=loadGloss(),b13=require('./v11_batch13_manual_gloss_r1.js');let added=0;const uncovered=new Set(),covered=new Set();for(const p of candidate.passages||[]){p.notes=Array.isArray(p.notes)?p.notes:[];const have=new Set(p.notes.map(n=>norm(n&&n.english)));for(const w of inv[p.id]||[]){if(have.has(norm(w)))continue;const g=findGloss(gloss,w);if(!g){uncovered.add(norm(w));continue;}const source=Object.prototype.hasOwnProperty.call(b13,g.base)?'Batch13 human-finalized Japanese gloss':'prior human-verified Japanese gloss'+(g.base===norm(w)?'':' via '+g.base);p.notes.push({english:norm(w),japanese:g.jp,kind:'unlearned_local_required',source});have.add(norm(w));covered.add(norm(w));added++;}}
