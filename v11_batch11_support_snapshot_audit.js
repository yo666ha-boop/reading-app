...
const rows=ps.map((p,i)=>({index:i+1,id:p.id,support:Array.isArray(p.supportNotes)?p.supportNotes.length:0,notes:Array.isArray(p.notes)?p.notes.length:0,title:p.title||''}));
