...
const payload={batch:'V11-B12',officialBefore:718,targetAfterFullGates:768,registered:false,passages:ps,finalSemanticRepairs:x.finalSemanticRepairs||[],finalSlashHumanReview:x.finalSlashHumanReview,easySupport:x.easySupport,verifiedGlossReuse:x.verifiedGlossReuse};
...
fs.writeFileSync('V11_BATCH12_RUNTIME_BUNDLE_STATUS.json',JSON.stringify({passages:ps.length,questions:ps.reduce((n,p)=>n+(p.questions||[]).length+(p.questionSetB||[]).length,0),supportNotes:ps.reduce((n,p)=>n+(p.supportNotes||[]).length,0),normalNotes:ps.reduce((n,p)=>n+(p.notes||[]).length,0),runtimeTextbooks:[...new Set(ps.map(p=>p.textbook))],registered:false,officialBefore:718,target:768,finalSlashHumanReview:x.finalSlashHumanReview,finalSemanticRepairs:x.finalSemanticRepairs||[],pass:true},null,2)+'\n');
console.log(JSON.stringify({passages:50,runtimeTextbooks:[...new Set(ps.map(p=>p.textbook))],registered:false,target:768,pass:true},null,2));
