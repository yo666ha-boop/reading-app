...
const ps=x.passages.map(p=>{
  ...
  q.textbook=q.anchor.textbook;q.grade=q.anchor.grade;q.section=q.anchor.unit;
...
fs.writeFileSync('V11_BATCH12_RUNTIME_BUNDLE_STATUS.json',JSON.stringify({passages:ps.length,questions:ps.reduce((n,p)=>n+(p.questions||[]).length+(p.questionSetB||[]).length,0),supportNotes:ps.reduce((n,p)=>n+(p.supportNotes||[]).length,0),normalNotes:ps.reduce((n,p)=>n+(p.notes||[]).length,0),registered:false,officialBefore:718,target:768,finalSlashHumanReview:x.finalSlashHumanReview,finalSemanticRepairs:x.finalSemanticRepairs||[],pass:true},null,2)+'\n');
console.log(JSON.stringify({passages:50,registered:false,target:768,pass:true},null,2));
