...
const files=[
  ...
  'v11_batch11_question_human_rewrite_r1.js','v11_batch11_question_human_rewrite_r2.js'
];
...
const out=all.map(p=>({id:p.id,title:p.title,textbook:p.textbook,grade:p.grade,section:p.section,level:p.level,wordCount:p.wordCount,targetWordBand:p.targetWordBand,sentences:p.sentences,fullTranslation:p.fullTranslation,slashRows:p.slashRows,materialData:p.materialData||null,freeWriteTask:p.freeWriteTask||null,questions:p.questions,questionSetB:p.questionSetB,authorReview:p.authorReview||null,semanticReviewR5:p.semanticReviewR5||null,humanReviewR2:p.humanReviewR2||null,questionStage:p.questionStage||null,questionHumanReview:p.questionHumanReview||null,registered:p.registered===true}));
