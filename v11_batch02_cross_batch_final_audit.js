...
try{
 ...
 run(s,'v11_batch01_passages_001_050.js');
 ...
 const crossOnly=severe.filter(x=>{const A=b1.some(p=>p.id===x.a),B=b1.some(p=>p.id===x.b);return A!==B});
 const out={generatedAt:new Date().toISOString(),batch01:b1.length,batch02:b2.length,total:all.length,bad,severe,crossOnly,finalPass:b1.length===50&&b2.length===50&&bad.length===0&&severe.length===0};
