...
const resultRe=/\b(after|afterward|then|finally|result|returned|reached|changed|added|learned|found|showed|ended|stayed|became|completed|arrived|sent|received)\b/i;
function pick(s,re,fallback,used){for(let i=0;i<s.length;i++){if(re.test(s[i])&&!used.has(i))return i;}for(const i of fallback){const k=Math.max(0,Math.min(s.length-1,i));if(!used.has(k))return k;}for(let i=0;i<s.length;i++)if(!used.has(i))return i;return Math.max(0,s.length-1);}
...
return {questionType:type,prompt,answer:jp,evidence:en,evidenceJp:jp,reason,questionReview:'B11_R2_CONTEXTUAL_HUMAN_PASS'};}
...
if(p.level==='YAMAGUCHI_EXAM'){
 ...
 q.push(make(p,'RESULT',take([Math.max(0,s.length-2)],resultRe),1));
 ...
 q.push(make(p,'RESULT',take([Math.max(0,s.length-3)],resultRe),1));
 q.push(make(p,'SUMMARY_FILL',take([Math.floor(s.length/2)]),1));
...
p.questions=q.slice(0,5);p.questionSetB=q.slice(5,10);p.questionStage='BATCH11_HUMAN_REWRITE_R2';p.questionHumanReview='FULL_50_PASSAGE_REVIEW_20260829';}
window.V11_BATCH11_QUESTION_REWRITE_R1_STATE={passages:all.length,questions:all.length*10,registered:false,version:'20260829-r2-contextual-human'};
})();
