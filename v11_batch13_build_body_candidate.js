...
module.exports=function build(){
 ...
 const semantic=read('v11_batch13_semantic_repair_r2.json');
 for(const r of semantic.repairs||[]){const p=passages.find(x=>x.id===r.id);if(!p)throw Error(`semantic target missing ${r.id}`);if(r.humanReviewed!==true)throw Error(`unreviewed semantic repair ${r.id}`);replaceExact(p,'body',r.findBody,r.replaceBody,r.id);replaceExact(p,'fullTranslation',r.findTranslation,r.replaceTranslation,r.id);p.humanSemanticReview='B13_G3_HUMAN_REVIEW_COMPLETE_R2';}
