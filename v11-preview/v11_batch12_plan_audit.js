...
const p12=JSON.parse(fs.readFileSync('v11_batch12_authoring_plan.json','utf8'));
...
const rows=p12.passages||[],old=[...(p10.passages||[]),...(p11.passages||[])];
assert(p12.registered===false,'Batch12 must remain unregistered');
assert(p12.currentOfficialTotal===718&&p12.targetAfterFullGates===768,'official/target mismatch');
...
const near=[];for(const a of rows)for(const b of old){const score=jac(a.title+' '+a.focus,b.title+' '+b.focus);if(score>=0.55)near.push({a:a.id,b:b.id,score:+score.toFixed(3)})}assert(!near.length,'Batch10/11 frame similarity '+JSON.stringify(near.slice(0,12)));
const within=[];for(let i=0;i<rows.length;i++)for(let j=i+1;j<rows.length;j++){const score=jac(rows[i].title+' '+rows[i].focus,rows[j].title+' '+rows[j].focus);if(score>=0.62)within.push({a:rows[i].id,b:rows[j].id,score:+score.toFixed(3)})}assert(!within.length,'Batch12 internal frame similarity '+JSON.stringify(within.slice(0,12)));
...
const out={batch:p12.batch,registered:false,currentOfficialTotal:718,targetAfterFullGates:768,passages:50,g1:17,g2:17,g3:16,g3Tiers:tiers,uniqueIds:ids.size,uniqueTitles:titles.size,priorNearFrames:near.length,internalNearFrames:within.length,yamaguchiMaterialItems:exam.length,pass:true};
fs.writeFileSync('V11_BATCH12_PLAN_AUDIT.json',JSON.stringify(out,null,2)+'\n');console.log(JSON.stringify(out,null,2));console.log('V11_BATCH12_PLAN_PASS');
