...
for(const p of ps){
 if(p.questionStage!=='BATCH10_HUMAN_REWRITE_R2')fail(p.id,'STAGE','question r2 not applied');if(p.questionHumanReview!=='FULL_50_PASSAGE_REVIEW_20260829')fail(p.id,'HUMAN_REVIEW','question human review marker missing');
...
const requiredTypes=['GIST','DETAIL','REASON','CONTENT_MATCH','RESULT','SUMMARY_FILL','EVIDENCE','PHRASE_FILL','MATERIAL_LINK'];for(const t of requiredTypes)if(!types[t])fail('B10','TYPE_MISSING',t);
const byGrade={};for(const p of ps){const g=String(p.grade);byGrade[g]=(byGrade[g]||0)+1;}
const out={generatedAt:new Date().toISOString(),passages:ps.length,byGrade,questionCount,expectedQuestions:500,uniquePrompts:prompts.size,evidenceLinked,answerLinked,freeWrites,yamaguchiMaterialLinks:yMaterial,questionTypes:types,failures,warnings,finalPass:ps.length===50&&questionCount===500&&prompts.size===500&&failures.length===0};fs.writeFileSync('V11_BATCH10_QUESTION_QUALITY_REPORT.json',JSON.stringify(out,null,2)+'\n');console.log(`B10 question r2 passages=${ps.length} questions=${questionCount} unique=${prompts.size} evidence=${evidenceLinked} answer=${answerLinked} freeWrites=${freeWrites} yMaterial=${yMaterial} failures=${failures.length} warnings=${warnings.length} final=${out.finalPass?'PASS':'FAIL'}`);if(!out.finalPass)process.exitCode=1;
