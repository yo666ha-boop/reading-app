...
const out={batch:'V11-B12',registered:false,status:'ASSEMBLED_AFTER_SEMANTIC_R2_HUMAN_REVIEW_COMPLETE',officialTotal:718,targetAfterFullGates:768,anchors,semanticRepairIds:[...new Set([...r1ids,...(r2.replacements||[]).map(x=>x.id)])],humanReviewedCount:reviewed.size,humanReviewPendingCount:0,passages:all};
fs.writeFileSync('v11_batch12_assembled_draft.json',JSON.stringify(out,null,2)+'\n');
console.log(JSON.stringify({passages:all.length,uniqueIds:new Set(all.map(x=>x.id)).size,distribution:dist,r1:r1ids.length,r2:(r2.replacements||[]).length,humanReviewed:reviewed.size,humanPending:0,yamaguchiWithMaterials:y.length,registered:false},null,2));