...
function ctxFor(batchNo,captured){const c={console:{log(){},warn(){},error(){}},setTimeout(){return 0},clearTimeout(){},Event:function(){},CustomEvent:function(){},fetch(){return Promise.reject(new Error('fetch disabled in static extractor'))}};c.window=c;c.globalThis=c;c.document={createElement(){return {dataset:{},style:{},set src(v){this._src=v},get src(){return this._src},set async(v){},onload:null,onerror:null}},head:{appendChild(){}},body:{appendChild(){}},querySelector(){return null},getElementById(){return null}};c.dispatchEvent=()=>{};c.V11_APPLY_EASY_SUPPORT_NOTES=()=>{};c.V11_REGISTER_PASSAGES=list=>{for(const p of list||[]){const b=text(p);if(p&&p.id&&words(b).length>=20)captured.set(String(p.id),{id:String(p.id),body:b,source:`runtime-b${String(batchNo).padStart(2,'0')}`});}return{extraPassages:batchNo*50,lastAdded:(list||[]).length}};vm.createContext(c);return c}
