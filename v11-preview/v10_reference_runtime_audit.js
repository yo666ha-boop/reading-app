...
(async()=>{
  ...
  for(const[g,b,d]of sets)for(const[sec,p]of Object.entries(d)){
    ...
    for(let i=0;i<r.length;i++){
      ...
      if(deSlash(en)!==norm(s[i]))errors.push(`${t}: English changed: ${deSlash(en)} <> ${norm(s[i])}`);
      ...
      for(const m of plain.matchAll(/\sto\s+[A-Za-z]/gi)){const idx=m.index||0,beforeText=plain.slice(Math.max(0,idx-4),idx+1);if(!/\/\s*$/.test(beforeText))errors.push(`${t}: to-boundary lacks slash: ${en}`)}
    }
  }
