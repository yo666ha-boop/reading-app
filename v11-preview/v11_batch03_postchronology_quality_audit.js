...
try{
 ...
 for(const p of ps){
   ...
   if((p.notes||[]).some(n=>!n||!n.english||!n.japanese||String(n.japanese).includes('最終注整理対象')))temporaryGlossPassages.push(p.id);
 }
 ...
 const questionsPending=ps.filter(p=>(p.questions||[]).length!==5||(p.questionSetB||[]).length!==5).length;
 ...
 const notePass=new Set(temporaryGlossPassages).size===0;
 const questionPass=questionsPending===0;
 const finalRegistrationReady=contentPass&&naturalnessPass&&notePass&&questionPass;
 const out={generatedAt:new Date().toISOString(),passages:ps.length,contentPass,lengthIssueCount:lengthIssues.length,lengthIssues,structureIssueCount:structureIssues.length,structureIssues,naturalnessPass,highSharedPairs:pairs.length,worstPairs:pairs.sort((a,b)=>b.score-a.score).slice(0,30),highCommonSentenceRatio:arcOnly.length,arcOnly,notePass,temporaryGlossPassageCount:new Set(temporaryGlossPassages).size,temporaryGlossPassages:[...new Set(temporaryGlossPassages)],questionPass,questionsPending,finalRegistrationReady};
 fs.writeFileSync('V11_BATCH03_POSTCHRONOLOGY_QUALITY_AUDIT.json',JSON.stringify(out,null,2)+'\n');
 console.log(JSON.stringify(out,null,2));
 if(!finalRegistrationReady)process.exitCode=1;
