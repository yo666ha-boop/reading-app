...
function wordBandOK(p){const g=Number(p.grade),lv=String(p.level||p.tier||'STANDARD').toUpperCase(),wc=wordCount(p.body||((p.sentences||[]).join(' ')));let a;if(g===1)a=lv==='LONG'?[135,165]:[90,125];else if(g===2)a=lv==='LONG'?[170,210]:[115,155];else if(g===3)a=lv==='YAMAGUCHI_EXAM'?[330,450]:lv==='LONG'?[240,330]:[150,230];else return false;return wc>=a[0]&&wc<=a[1];}
for(const p of ps){
  ...
  if(p.finalSlashHumanReview!=='B12_FINAL_SLASH_HUMAN_REVIEW_R8_LENGTH_REPAIRED')throw new Error(p.id+' final slash human review missing');
  if(!wordBandOK(p))throw new Error(p.id+' word-count band invalid');
...
if(!bundle.lengthRepairR8||bundle.lengthRepairR8.ids.length!==12)throw new Error('Batch12 R8 length repair contract missing');
...
window.V11_BATCH12_STATE={...st,totalWithBaseline,batch12Passages:50,registered:true,humanReviewedPassages:50,humanReviewedQuestions:500,slashReviewedPassages:50,normalNotes:ps.reduce((n,p)=>n+(p.notes||[]).length,0),supportNotes:ps.reduce((n,p)=>n+(p.supportNotes||[]).length,0),version:'20260901-b12-r8-final'};
