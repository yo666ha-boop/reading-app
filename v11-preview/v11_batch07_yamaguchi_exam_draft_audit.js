...
vm.runInContext(fs.readFileSync('v11_batch07_yamaguchi_exam_evidence_sync.js','utf8'),sandbox,{filename:'v11_batch07_yamaguchi_exam_evidence_sync.js'});
const ps=sandbox.window.V11_BATCH07_YAMAGUCHI_EXAM_DRAFTS||[];
const syncState=sandbox.window.V11_BATCH07_YAMAGUCHI_EVIDENCE_SYNC_STATE||{};
...
if((syncState.missing||[]).length) errors.push(`evidence-sync-missing:${syncState.missing.length}`);
for(const p of ps){
...
const report={pass:errors.length===0,count:ps.length,evidenceSync:syncState,errors,details};
...
console.log(`EVIDENCE SYNC passages=${syncState.passages||0} questions=${syncState.questions||0} synced=${syncState.synced||0} missing=${(syncState.missing||[]).length}`);
