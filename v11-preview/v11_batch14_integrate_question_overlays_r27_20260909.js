...
const specs={
'V11-B14-G3-004':['v11_batch14_g3_004_005_questions_r16_20260908.json',0],
'V11-B14-G3-005':['v11_batch14_g3_004_005_questions_r16_20260908.json',1],
...
function arrays(x){if(Array.isArray(x))return [x]; if(!x||typeof x!=='object')return []; return Object.values(x).filter(Array.isArray)}
for(const [id,s] of Object.entries(specs)){let qs=[]; for(const f of s.filter(x=>typeof x==='string')){const x=JSON.parse(fs.readFileSync(f,'utf8')); for(const ar of arrays(x)) qs.push(...ar.filter(q=>q&&typeof q==='object'&&('answer'in q||'evidence'in q)));} const p=map.get(id); if(!p)throw Error('missing '+id); const uniq=[]; const seen=new Set(); for(const q of qs){const k=JSON.stringify(q);if(!seen.has(k)){seen.add(k);uniq.push(q)}} if(uniq.length!==10)throw Error(id+' questions='+uniq.length); p.questions=uniq.slice(0,5);p.questionSetB=uniq.slice(5);}
fs.writeFileSync('v11_batch14_assembled_draft.json',JSON.stringify(a,null,2)+'\n');
console.log('integrated',Object.keys(specs).length,'G3 passages');
