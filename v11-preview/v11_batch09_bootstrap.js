...
(async()=>{try{for(const f of files)await load(f);if(!window.V11_BATCH09_LOADED)throw new Error('Batch09 registration did not complete');await load('v11_batch10_bootstrap.js');await new Promise((resolve,reject)=>{const t=Date.now();(function wait(){if(window.V11_BATCH10_LOADED)return resolve();if(window.V11_BATCH10_BOOTSTRAP_ERROR)return reject(new Error(window.V11_BATCH10_BOOTSTRAP_ERROR));if(Date.now()-t>120000)return reject(new Error('Batch10 bootstrap timeout'));setTimeout(wait,20)})();});window.V11_BATCH09_BOOTSTRAP_STATE={version:'20260829-human-r1-b10-chain',files:files.length+1,loaded:true,total:window.V11_BATCH10_STATE&&window.V11_BATCH10_STATE.totalWithBaseline||window.V11_BATCH09_STATE&&window.V11_BATCH09_STATE.totalWithBaseline};if(typeof window.render==='function')window.render();}catch(e){window.V11_BATCH09_BOOTSTRAP_ERROR=String(e&&e.stack||e);console.error(e);throw e;}finally{window.V11_BATCH09_BOOTSTRAP_LOADING=false;}})();
})();
