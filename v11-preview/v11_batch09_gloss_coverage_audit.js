...
Object.assign(gloss,vm.runInNewContext('('+mm[1]+')'));
...
const possessive=w=>w.endsWith("'s")?w.slice(0,-2):null,covered=[],missing=[];
for(const w of words){const b=possessive(w);if(gloss[w])covered.push([w,gloss[w],'verified-prior']);else if(b&&gloss[b])covered.push([w,gloss[b]+'の','verified-prior-possessive']);else if(names.has(w)||b&&names.has(b))covered.push([w,'固有名詞','proper-name']);else missing.push(w);}
...
fs.writeFileSync('V11_BATCH09_GLOSS_COVERAGE_AUDIT.json',JSON.stringify(report,null,2)+'\n');
console.log(`Batch09 gloss distinct=${words.length} covered=${covered.length} missing=${missing.length} coverage=${(report.coverage*100).toFixed(1)}%`);
console.log('missing sample='+missing.slice(0,80).join(','));