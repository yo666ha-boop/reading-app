...
const data=load(['v10_data_sunshine_g3_program3.js','v10_data_sunshine_g3_program3_fix.js','v10_data_sunshine_g3_program3_fix2.js','v10_data_sunshine_g3_program3_fix3.js'],'V10_PASSAGES_G3_SS');
const meta=load(['v10_interaction_metadata_sun_g3_p3.js'],'V10_INTERACTION_META_G3_SS_P3');
const expected=['PROGRAM 3-1','PROGRAM 3-2','PROGRAM 3-3'];const errors=[];let bq=0;
...
const p32=[' attracted ','take part in',' incredible ',' boring ',' progress ',' sportswear ',' beyond ',' huge ',' sunburn ',' runner ',' typical ','take care of',' trainer ',' athlete ',' weather '];
const p33=[' development ',' cause ',' breathing ',' recommend ',' base ',' training ',' failure ',' diet ',' carefully ',' importance ',' nutritional '];
const p4=['sign language',' sign ',' gesture ',' deaf ',' hearing ',' communicate ',' communication ',' hand ',' finger ',' facial ',' expression '];
const future={'PROGRAM 3-1':[...p32,...p33,...p4],'PROGRAM 3-2':[...p33,...p4],'PROGRAM 3-3':p4};
...
const forbidden=['works','wears','takes','talks','wants','comes','checks','needs','explains','follows','supports','helps','players','rules','teams','mascots','athletes','trainers','runners'];
...
console.log(`G3 SS P3 AUDIT passages=${Object.keys(data).length}/${expected.length} alternate_questions=${bq}`);
console.log('G3 SS P3 CANONICAL v7=PROGRAM3 3-1 / 3-2 / 3-3 exact source partitions; PROGRAM4 sign-language vocabulary gated');
if(errors.length){console.error(`G3 SS P3 AUDIT FAIL ${errors.length}`);for(const e of errors)console.error(`- ${e}`);process.exit(1)}
console.log('G3 SS P3 AUDIT PASS: Program 3 passages, slash rows, A/B questions, evidence links, release flags, morphology guard, chronological partitions, and Program 4 future-vocab gate are consistent.');
