...
function applyTranslationBoundaryRepair(passages,file,marker){const d=read(file);if(d.humanReviewed!==true||d.registered!==false)throw Error(`translation repair metadata ${file}`);for(const r of d.repairs||[]){const p=passages.find(x=>x.id===r.id);if(!p)throw Error(`translation repair target missing ${r.id}`);replaceExact(p,'fullTranslation',r.find,r.replace,r.id);p.translationBoundaryHumanReview=marker;}}
module.exports=function build(){
 ...
 applyTranslationBoundaryRepair(passages,'v11_batch13_translation_boundary_repair_r4_slash.json','B13_G3_SLASH_BOUNDARY_HUMAN_R4');
 applyTranslationBoundaryRepair(passages,'v11_batch13_translation_boundary_repair_r5_naturalness.json','B13_SLASH_HUMAN_NATURALNESS_R5');
