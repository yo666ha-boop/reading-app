...
const data=load(['v10_data_newhorizon_g2_unit7.js'],'V10_PASSAGES_G2_NH');
const meta=load(['v10_interaction_metadata_nh_g2_u7.js'],'V10_INTERACTION_META_G2_NH_U7');
const expected=['Unit 7-1','Unit 7-2','Unit 7-3','Unit 7-4'];const errors=[];let bq=0;
...
if(Object.keys(meta).length!==expected.length)errors.push(`NH2 U7 interaction expected ${expected.length}, got ${Object.keys(meta).length}`);
function text(section){return ` ${(data[section].sentences||[]).join(' ').toLowerCase()} `}
const rt1=[' protect ',' material ',' cover ',' marble ',' jewel ','because of',' government ',' turn ',' emperor ',' face ',' architecture ',' wife ','in memory of',' pollution ','from far away'];
...
const forbidden=['selected','covered','marbles','jewels','pollutions','craters','clouds','campaigns','cleanups','trails','tourists','heritages','specieses'];
...
console.log(`G2 NH U7 AUDIT passages=${Object.keys(data).length}/${expected.length} alternate_questions=${bq}`);
console.log('G2 NH U7 CANONICAL v7=Part1,2 shared pool split into 7-1/7-2 + ReadThink1 -> 7-3 + ReadThink2 -> 7-4; textbook-body proper nouns preserved');
if(errors.length){console.error(`G2 NH U7 AUDIT FAIL ${errors.length}`);for(const e of errors)console.error(`- ${e}`);process.exit(1)}
console.log('G2 NH U7 AUDIT PASS: Unit 7 passages, slash rows, A/B questions, evidence links, release flags, morphology guard, chronological vocab gates, and post-Unit7 future-vocab gate are consistent.');
