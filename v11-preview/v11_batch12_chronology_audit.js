...
const loader=`function loadDraft(){const build=require('./v11_batch12_build_final_candidate.js');const applyGloss=require('./v11_batch12_apply_verified_gloss.js');let x=build();x=applyGloss(x);if(!x||!Array.isArray(x.passages)||x.passages.length!==50)throw Error('Batch12 candidate not 50');const ps=x.passages.map(p=>{const q=JSON.parse(JSON.stringify(p));const a=q.anchor||{};q.textbook=a.textbook==='Sunshine'?'サンシャイン':a.textbook==='New Horizon'?'ニューホライズン':a.textbook;q.grade=Number(a.grade);q.section=a.unit;q.semanticRewrite=q.humanSemanticReview||'B12_HUMAN_SEMANTIC_COMPLETE';return q;});return ps;}`;
