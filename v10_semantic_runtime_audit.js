...
async function waitFor(fn,ms=25000){const t=Date.now();while(Date.now()-t<ms){if(fn())return;await new Promise(r=>setTimeout(r,50))}throw new Error('runtime load timeout')}
(async()=>{
 ...
 const w=dom.window;
 await waitFor(()=>w.V10_INTERACTION_META_SEMANTIC_REPAIRS_161_168&&w.V10_PASSAGES_G3_NH&&w.V10_PASSAGES_G3_NH['Unit 6-4']&&w.V10_PASSAGES_G3_NH['Unit 6-4'].title==='How One Coat Connects Several Countries');
 const sets=[
 ...
 const markers=[
  [w.V10_PASSAGES_G2_NH,'Unit 7-4','Keeping the Mountain Trail Clean'],
  [w.V10_PASSAGES_G3_SS,'PROGRAM 6-3','One Reusable Mug and Less Plastic Waste'],
  [w.V10_PASSAGES_G3_NH,'Unit 2-2','How Long the Designer Has Used a Recycling Idea'],
  [w.V10_PASSAGES_G3_NH,'Unit 4-4','Delivering Relief by Bicycle'],
  [w.V10_PASSAGES_G3_NH,'Unit 6-4','How One Coat Connects Several Countries']
 ];
 for(const[d,s,t]of markers)if(!d||!d[s]||d[s].title!==t)errors.push(`runtime marker failed ${s}: expected ${t}`);
 console.log(`SEMANTIC RUNTIME AUDIT total=${total}/168 A_questions=${aq} B_questions=${bq}`);
