...
for(const r of repair.replacements||[]){const p=byId.get(r.id);if(!p)throw new Error('repair id missing '+r.id);if(r.body)p.body=r.body;if(r.fullTranslation)p.fullTranslation=r.fullTranslation;if(r.bodyReplace){if(!p.body.includes(r.bodyReplace.from))throw new Error('bodyReplace source missing '+r.id);p.body=p.body.replace(r.bodyReplace.from,r.bodyReplace.to)}if(r.translationReplace){if(!p.fullTranslation.includes(r.translationReplace.from))throw new Error('translationReplace source missing '+r.id);p.fullTranslation=p.fullTranslation.replace(r.translationReplace.from,r.translationReplace.to)}if(r.materialsPatch)p.materials=Object.assign({},p.materials||{},r.materialsPatch);if(r.freeWriteTask)p.freeWriteTask=r.freeWriteTask;p.semanticRepair='R1_APPLIED'}
...
const repaired=(repair.replacements||[]).map(x=>x.id);if(repaired.length!==4||repaired.some(id=>!byId.get(id)?.semanticRepair))throw new Error('semantic R1 not fully applied');
...
const out={batch:'V11-B12',registered:false,status:'ASSEMBLED_AFTER_SEMANTIC_R1_HUMAN_REVIEW_REQUIRED',officialTotal:718,targetAfterFullGates:768,anchors,semanticRepairIds:repaired,passages:all};fs.writeFileSync('v11_batch12_assembled_draft.json',JSON.stringify(out,null,2)+'\n');console.log(JSON.stringify({passages:all.length,uniqueIds:new Set(all.map(x=>x.id)).size,distribution:dist,repaired,yamaguchiWithMaterials:y.length,anchors:[...new Set(all.map(x=>`${x.anchor.textbook}|${x.anchor.grade}|${x.anchor.unit}`))],registered:false},null,2));
