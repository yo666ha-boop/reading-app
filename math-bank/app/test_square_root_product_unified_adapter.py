...
def main():
    p=copy.deepcopy(make_base()[0]); p["id"]="U-ROOT-PROD"; p["question"]="√2×√8を計算しなさい。"; p["answer"]="4"; p["choices"]=None; p["figure_refs"]=[]; p["source"]["is_generated_variant"]=False; p["source"]["parent_id"]=None; p["variant_group"]=None
    rows,prov,reason=generate_parent(p,3,NOW)
    assert reason.startswith("specialized:square_root_simplification:square_root_product_integer_exact")
    ...
    for row,ev in zip(rows,prov):
        ...
        assert "square_root_product_exact_perfect_square_and_square_identity" in ev["verification_evidence"]
        nums=tuple(__import__("re").findall(r"\d+",row["question"])); assert nums not in surfaces; surfaces.add(nums)
    print("PASS_SQUARE_ROOT_PRODUCT_UNIFIED_ADAPTER")
