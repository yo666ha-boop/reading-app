...
"""Fail-closed exact engine for narrow Pythagorean right-triangle parents."""
...
from safe_pythagorean_leg_variant_engine import generate as generate_leg_inverse
...
def can_generate(parent:dict)->tuple[bool,str]:
    rows,_,reason=generate_leg_inverse(parent,1)
    if rows:
        return True,reason
...
def generate(parent:dict,count:int)->tuple[list[dict],list[dict],str]:
    ...
    rows,evidence,reason=generate_leg_inverse(parent,count)
    if rows:
    ...
    parent_signature=(str(pa),str(pb)); seen=set(); rows=[]; evidence=[]
    for index in range(count):
        ...
        if a*a+b*b!=c*c or c*c-a*a!=b*b or c*c-b*b!=a*a:
            raise AssertionError("pythagorean identity failed")
        replacement=f"直角をはさむ2辺が{a}cmと{b}cm"
        new_question=q[:match.start()]+replacement+q[match.end():]
        rows.append({"question":new_question,"answer":f"{c}cm","explanation":f"三平方の定理より、斜辺²={a}²+{b}²={c*c}。したがって斜辺は{c}cm。逆算でも2辺を確認済み。","numeric_signature":signature})
        evidence.append({"parent_sha256":_parent_sha(parent),"method":"pythagorean_hypotenuse_exact_square_sum_and_two_inverse_identities","parent_recalculation":f"{pa}^2+{pb}^2={pc}^2","variant_recalculation":f"{a}^2+{b}^2={c}^2","independent_check":f"{c}^2-{a}^2={b}^2 AND {c}^2-{b}^2={a}^2 PASS"})
