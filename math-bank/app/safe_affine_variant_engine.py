...
def _parse_parent(parent: dict):
    ...
    formula=fm[0]; xmatch=xm[0]; a_text=formula.group("a"); a=Fraction(1) if a_text in (None,"","+") else _fraction(a_text); b=Fraction(0)
    if formula.group("b"):
...
def can_generate(parent: dict)->tuple[bool,str]:
    for fn in (generate_affine_two_points, generate_linear_inequality, generate_linear_equation, generate_affine_intercept, generate_affine_x_from_y, generate_direct_proportion_constant, generate_direct_proportion_value):
...
def generate(parent:dict,count:int)->tuple[list[dict],list[dict],str]:
    ...
    q=_norm(parent.get("question")); seed=int(_parent_sha(parent)[:12],16); rows=[]; evidence=[]; numeric_signatures=set(); parent_signature=(_fraction_text(a),_fraction_text(b),_fraction_text(x))
    for index in range(1,count+1):
        na=Fraction((abs(a.numerator)+1+index)*(-1 if ((seed>>index)&1) else 1),max(1,a.denominator)); nb=b+Fraction(2+index+((seed>>(index+5))&3)); nx=x+Fraction(1+index+((seed>>(index+11))&3))
        if na==0: na=Fraction(index+1)
        signature=(_fraction_text(na),_fraction_text(nb),_fraction_text(nx))
        if signature==parent_signature or signature in numeric_signatures: nx+=index+1; signature=(_fraction_text(na),_fraction_text(nb),_fraction_text(nx))
        ...
        a_text=_fraction_text(na); lhs="y=x" if na==1 else "y=-x" if na==-1 else f"y={a_text}x"
        if nb>0: lhs+=f"+{_fraction_text(nb)}"
        elif nb<0: lhs+=_fraction_text(nb)
        new_question=q; replacements=[(formula.start("formula"),formula.end("formula"),lhs),(xmatch.start("x"),xmatch.end("x"),_fraction_text(nx))]
