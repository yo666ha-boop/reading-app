...
NOW="2026-08-23T09:10:00Z"


def main():
    parent=copy.deepcopy(make_base()[0])
    parent["id"]="U-PERCENT-CHANGE"
    parent["question"]="ある人数が80人から100人に増えました。何%増えましたか。"
    parent["answer"]="25%"
    parent["choices"]=[]
    parent["figure_refs"]=[]
    ...
    assert reason.startswith("specialized:percentage:simple_integer_percent_change_exact")
    ...
    for row,p in zip(rows,prov):
        ...
        assert "delta*100 == percent*original PASS" in p["verification_evidence"]
    print("PASS_PERCENT_CHANGE_UNIFIED_PARENT_SHA_METADATA_AUDIT_AND_IDENTITY")
