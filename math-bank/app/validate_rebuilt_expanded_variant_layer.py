...
def configure_rebuilt_mode(base_records: list[dict], freeze_manifest: dict) -> str:
    ...
    if not frozen.get("pass"): raise ValueError("immutable rebuilt originals manifest validation did not PASS")
    anchor = str(freeze_manifest.get("combined_payload_sha256", "")).strip().lower()
    if len(anchor) != 64: raise ValueError("rebuilt combined payload SHA-256 missing/invalid")
    legacy.BASE_CANONICAL_SHA256 = anchor
    legacy.EXPECTED_BASE_TOTAL = EXPECTED_TOTAL
    legacy.EXPECTED_ORIGINALS = EXPECTED_TOTAL
    legacy.EXPECTED_BASELINE_VARIANTS = 0
    legacy.EXPECTED_ORIGINAL_BY_SOURCE = EXPECTED_COUNTS.copy()
    return anchor
...
def load_and_validate(base_path: Path, freeze_manifest_path: Path, layer_path: Path, *, require_full_parent_coverage: bool=False) -> dict:
    ...
    result["mode"]="REBUILT_1271_DYNAMIC_IMMUTABLE_BASE"
...
def main()->int:
    ...
    except Exception as exc: print(f"FAIL_CLOSED: {exc}"); return 13
