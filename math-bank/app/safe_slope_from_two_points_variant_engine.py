...
"""Fail-closed exact slope engine for a line through two explicit integer points."""
...
def _parse(parent:dict):
    ...
    if any(t in q for t in ("グラフ","平行","垂直","切片","方程式","関数の式")):return None
    ...
    if _ans(parent.get("answer"))!=m:return None
    if Fraction(y2-y1,1)!=m*Fraction(x2-x1,1):return None
...
def can_generate(parent:dict):
    ...
    return False,"slope_parent_not_exactly_parsed_and_verified"
...
def generate(parent:dict,count:int):
    ...
    ps,px1,py1,px2,py2,pm=parsed;q=_norm(parent.get("question"));seed=int(_sha(parent)[:12],16)
    rows=[];ev=[];seen=set()
    for idx in range(1,count+1):
        x1=-3+((seed>>(idx*3))+idx)%7
        dx=1+((seed>>(idx*5+2))+idx)%5
        m_num=(-4+((seed>>(idx*7+1))+idx*3)%9) or 2
        x2=x1+dx;y1=-6+((seed>>(idx*4+5))+idx*2)%13;y2=y1+m_num*dx
        sig=(x1,y1,x2,y2)
        ...
        nq=q
        replacements=[(ps[0].start(),ps[0].end(),f"({x1},{y1})"),(ps[1].start(),ps[1].end(),f"({x2},{y2})")]
