...
PARENT_CONTRACT_FIELDS=("grade","unit","skill","difficulty","format","question_format","taxonomy")
AUDIT_TRUE_FIELDS=("problem_answer_verified","structure_verified","figure_refs_verified")
...
def _assert_variant_parent_contract(parent: dict, variant: dict, provenance: dict, engine_name: str) -> None:
    for field in PARENT_CONTRACT_FIELDS:
        if variant.get(field) != parent.get(field): raise AssertionError(f"{engine_name} parent contract mismatch: {field}")
    ...
    for field in AUDIT_TRUE_FIELDS:
        if audit.get(field) is not True: raise AssertionError(f"{engine_name} parent contract mismatch: audit.{field}")
    expected_sha=parent_record_sha256(parent)
    if provenance.get("parent_id") != parent.get("id"): raise AssertionError(f"{engine_name} provenance parent_id mismatch")
    if provenance.get("parent_record_sha256") != expected_sha: raise AssertionError(f"{engine_name} provenance parent fingerprint mismatch")
    if provenance.get("independent_recalculation") is not True: raise AssertionError(f"{engine_name} independent recalculation evidence missing")
    if not provenance.get("verification_evidence"): raise AssertionError(f"{engine_name} verification evidence missing")
...
def _adapt_specialized(parent: dict, count: int, now: str) -> tuple[list[dict], list[dict], str]:
    ...
    for engine_name,engine in SPECIALIZED_ENGINES:
        ...
        if not rows:
            reasons.append(f"{engine_name}:{reason}"); continue
    ...
    if legacy_variants:
        if len(legacy_variants)!=len(legacy_prov): raise AssertionError("legacy exact adapter returned incomplete provenance set")
        for row,p in zip(legacy_variants,legacy_prov):
            p["generator"]="generate_all_safe_verified_variants.py"; p["generation_method"]="legacy_exact_adapter:"+str(p.get("generation_method") or legacy_reason); _assert_variant_parent_contract(parent,row,p,"legacy_exact")
        return legacy_variants,legacy_prov,f"legacy:{legacy_reason}"
    reasons.append(f"legacy:{legacy_reason}"); return [],[],"unsupported_all_safe_engines:"+"|".join(reasons)
...
def generation_request(existing_count:int,*,minimum_per_parent:int,safe_target_per_parent:int)->tuple[int,int]:
    ...
    if minimum_per_parent not in (1,2,3): raise ValueError("minimum_per_parent must be 1, 2, or 3")
    if safe_target_per_parent not in (1,2,3): raise ValueError("safe_target_per_parent must be 1, 2, or 3")
    if safe_target_per_parent<minimum_per_parent: raise ValueError("safe_target_per_parent must be >= minimum_per_parent")
...
def main()->int:
    ...
    final_report=validate_layer(base,out_layer["variants"],out_layer["provenance"],require_full_parent_coverage=False); Path(ns.output).write_text(json.dumps(out_layer,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    if ns.manual_queue:
        queue_payload={"schema_version":"1.0","base_canonical_sha256":BASE_CANONICAL_SHA256,"recorded_at_utc":now,"policy":"Actual BASE parents only; every task is fingerprint-bound and contains no generated question. Manual output must pass the same strict validator before counting as coverage.","minimum_per_parent":ns.target_per_parent,"safe_target_per_parent":ns.safe_target_per_parent,"manual_parent_count":len(manual_queue),"tasks":manual_queue}; Path(ns.manual_queue).write_text(json.dumps(queue_payload,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
