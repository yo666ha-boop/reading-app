...
"""Unified fail-closed driver for proven safe math variant engines."""
...
SPECIALIZED_ENGINES=(("absolute_value",generate_absolute_value),("affine",generate_affine),("symmetric_linear_system",generate_symmetric_linear_system),("inverse_proportion",generate_inverse_proportion),("pythagorean_hypotenuse",generate_pythagorean_hypotenuse),("quadratic_square_equation",generate_quadratic_square_equation),("square_root_simplification",generate_square_root_simplification),("integer_power",generate_integer_power),("decimal_arithmetic",generate_decimal_arithmetic),("fraction_arithmetic",generate_fraction_arithmetic),("single_draw_probability",generate_probability),("two_dice_sum_probability",generate_two_dice_sum_probability),("gcd",generate_gcd),("lcm",generate_lcm),("prime_factorization",generate_prime_factorization),("proportion",generate_proportion),("percentage",generate_percentage),("discount_price",generate_discount_price),("tax_inclusive_price",generate_tax_inclusive_price),("data_range",generate_data_range),("median",generate_median),("mode",generate_mode),("relative_frequency",generate_relative_frequency),("interquartile_range",generate_interquartile_range),("simple_average",generate_average),("rectangle_area",generate_rectangle_area),("rectangle_perimeter",generate_rectangle_perimeter),("circle_area_pi_3_14",generate_circle_area),("circle_circumference_pi_3_14",generate_circle_circumference),("square_area",generate_square_area),("triangle_area",generate_triangle_area),("parallelogram_area",generate_parallelogram_area),("trapezoid_area",generate_trapezoid_area),("rectangular_prism_volume",generate_rectangular_prism_volume),("cube_volume",generate_cube_volume),("cylinder_volume_pi_3_14",generate_cylinder_volume),("cone_volume_pi_3_14",generate_cone_volume),("sphere_pi_3_14",generate_sphere),("speed_distance",generate_speed_distance))
PARENT_CONTRACT_FIELDS=("grade","unit","skill","difficulty","format","question_format","taxonomy");AUDIT_TRUE_FIELDS=("problem_answer_verified","structure_verified","figure_refs_verified")
def _verification_evidence(n,e):return f"engine={n}; "+json.dumps(e,ensure_ascii=False,sort_keys=True,separators=(',',':'))
def _assert_variant_parent_contract(parent,variant,provenance,engine_name):
    for f in PARENT_CONTRACT_FIELDS:
        if variant.get(f)!=parent.get(f):raise AssertionError(f"{engine_name} parent contract mismatch: {f}")
    if variant.get('choices')!=parent.get('choices'):raise AssertionError(f"{engine_name} parent contract mismatch: choices")
    if list(variant.get('figure_refs') or [])!=list(parent.get('figure_refs') or []):raise AssertionError(f"{engine_name} parent contract mismatch: figure_refs")
    source=variant.get('source') if isinstance(variant.get('source'),dict) else {}
    if source.get('parent_id')!=parent.get('id'):raise AssertionError(f"{engine_name} parent contract mismatch: parent_id")
    audit=variant.get('audit') if isinstance(variant.get('audit'),dict) else {}
    for f in AUDIT_TRUE_FIELDS:
        if audit.get(f) is not True:raise AssertionError(f"{engine_name} parent contract mismatch: audit.{f}")
    expected=parent_record_sha256(parent)
    if provenance.get('parent_id')!=parent.get('id') or provenance.get('parent_record_sha256')!=expected:raise AssertionError(f"{engine_name} provenance parent mismatch")
    if provenance.get('independent_recalculation') is not True or not provenance.get('verification_evidence'):raise AssertionError(f"{engine_name} verification evidence missing")
def _adapt_specialized(parent,count,now):
    ...
    for engine_name,engine in SPECIALIZED_ENGINES:
        ...
        if not rows:reasons.append(f'{engine_name}:{reason}');continue
        if len(rows)!=count or len(evidence_rows)!=count:raise AssertionError(f'{engine_name} returned incomplete sibling set')
        parent_sig=tuple(numeric_tokens(str(parent.get('question') or '')));sibling_sigs=set();variants=[];provenance=[]
        for index,(row,evidence) in enumerate(zip(rows,evidence_rows),1):
            question=str(row.get('question') or '');answer=row.get('answer')
            if not question or answer in (None,''):raise AssertionError(f'{engine_name} returned blank question/answer')
            sig=tuple(numeric_tokens(question))
            if sig==parent_sig or sig in sibling_sigs:raise AssertionError(f'{engine_name} numeric surface collision')
            sibling_sigs.add(sig);method=f'specialized-{engine_name}-{reason}';vid=variant_id(parent,method,index);full=make_variant_base(parent,vid);full['question']=question;full['answer']=answer;full['explanation']=str(row.get('explanation') or '')
    ...
    rows,prov,reason=generate_legacy_exact(parent,count,now)
    if rows:
        if len(rows)!=len(prov):raise AssertionError('legacy exact adapter returned incomplete provenance set')
        for row,p in zip(rows,prov):p['generator']='generate_all_safe_verified_variants.py';p['generation_method']='legacy_exact_adapter:'+str(p.get('generation_method') or reason);_assert_variant_parent_contract(parent,row,p,'legacy_exact')
        return rows,prov,f'legacy:{reason}'
...
def main():
    ...
    base=load_records(Path(ns.base));_,originals,_=base_gate(base);variants,provenance_rows,_=load_layer(Path(ns.expanded));validate_layer(base,variants,provenance_rows,require_full_parent_coverage=False);counts=defaultdict(int)
    ...
    out={'schema_version':'1.0','base_canonical_sha256':BASE_CANONICAL_SHA256,'variants':variants+generated,'provenance':provenance_rows+generated_provenance};final=validate_layer(base,out['variants'],out['provenance'],require_full_parent_coverage=False);Path(ns.output).write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    if ns.manual_queue:Path(ns.manual_queue).write_text(json.dumps({'schema_version':'1.0','base_canonical_sha256':BASE_CANONICAL_SHA256,'recorded_at_utc':now,'policy':'Actual BASE parents only; every task is fingerprint-bound and contains no generated question. Manual output must pass the same strict validator before counting as coverage.','minimum_per_parent':ns.target_per_parent,'safe_target_per_parent':ns.safe_target_per_parent,'manual_parent_count':len(manual_queue),'tasks':manual_queue},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
