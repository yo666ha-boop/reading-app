...
from safe_regular_polygon_sides_from_exterior_angle_variant_engine import generate as generate_regular_polygon_sides_from_exterior_angle
...
from safe_two_coin_heads_probability_variant_engine import generate as generate_two_coin_heads_probability
...
SPECIALIZED_ENGINES = (
    ...
    ("two_coin_heads_probability", generate_two_coin_heads_probability),
    ...
    ("regular_polygon_sides_from_exterior_angle", generate_regular_polygon_sides_from_exterior_angle),
...
def _verification_evidence(name, evidence):
    return f"engine={name}; " + json.dumps(evidence, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
...
def _assert_variant_parent_contract(parent, variant, provenance, engine_name):
    ...
    if provenance.get("parent_id") != parent.get("id") or provenance.get("parent_record_sha256") != expected:
        raise AssertionError(f"{engine_name} provenance parent mismatch")
    if provenance.get("independent_recalculation") is not True or not provenance.get("verification_evidence"):
