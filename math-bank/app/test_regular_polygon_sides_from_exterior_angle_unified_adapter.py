...
def main():
    ...
    assert reason.startswith("specialized:regular_polygon_exterior_angle:regular_polygon_sides_from_single_exterior_angle_exact")
    ...
    for row,evidence in zip(rows,prov):
        ...
        assert "engine=regular_polygon_exterior_angle" in evidence["verification_evidence"]
        assert row["answer"].endswith("角形")
    print("PASS_REGULAR_POLYGON_SIDES_FROM_EXTERIOR_UNIFIED_PARENT_CONTRACT")
