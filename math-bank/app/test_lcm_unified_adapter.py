...
def main() -> None:
    ...
    parent["id"] = "U-LCM"
    parent["question"] = "12と18の最小公倍数を求めなさい。"
    parent["answer"] = "36"
    parent["choices"] = None
    parent["figure_refs"] = []
    ...
    assert reason.startswith("specialized:lcm:")
    ...
    for row, evidence in zip(rows, prov):
        ...
        assert "engine=lcm" in evidence["verification_evidence"]
        assert "PASS" in evidence["verification_evidence"]
    print("PASS_LCM_UNIFIED_ADAPTER_PARENT_SHA_METADATA_AUDIT_AND_INDEPENDENT_RECALCULATION")
