...
def main():
    p=copy.deepcopy(make_base()[0]); p["id"]="U-QUAD-ROOTS"; p["question"]="二次方程式 x²-5x+6=0 を解きなさい。"; p["answer"]="x=2, x=3"; p["choices"]=None; p["figure_refs"]=[]; p["source"]["is_generated_variant"]=False; p["source"]["parent_id"]=None; p["variant_group"]=None
    rows,prov,reason=generate_parent(p,3,NOW); assert reason.startswith("specialized:quadratic_square_equation:quadratic_monic_two_integer_roots_exact"); assert len(rows)==len(prov)==3
    ...
    for row,ev in zip(rows,prov):
        ...
        assert "quadratic_monic_integer_roots_discriminant_vieta_and_substitution" in ev["verification_evidence"]
    print("PASS_QUADRATIC_INTEGER_ROOTS_UNIFIED_ADAPTER")
