...
NOW="2026-08-23T10:05:00Z"

def main():
    parent=copy.deepcopy(make_base()[0])
    parent["id"]="U-RATIO-SIMPLIFY"
    parent["question"]="18:24を最も簡単な整数の比にしなさい。"
    parent["answer"]="3:4"
    parent["choices"]=[]
    parent["figure_refs"]=[]
    ...
    assert reason.startswith("specialized:proportion:two_integer_ratio_simplification_exact")
    ...
    for row,p in zip(rows,prov):
        ...
        assert "engine=proportion" in p["verification_evidence"]
        assert "gcd(simplified)==1" in p["verification_evidence"]
    print("PASS_RATIO_SIMPLIFICATION_UNIFIED_PARENT_SHA_METADATA_AUDIT_AND_IDENTITY")
