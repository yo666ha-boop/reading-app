...
NOW="2026-08-24T00:45:00Z"
def main():
    parent=copy.deepcopy(make_base()[0]); parent["id"]="U-TRAP-H"
    parent["question"]="上底6cm、下底10cm、面積48cm²の台形の高さを求めなさい。"; parent["answer"]="6cm"
    ...
    rows,prov,reason=generate_parent(parent,3,NOW); assert reason.startswith("specialized:trapezoid_area:trapezoid_height_from_area_exact")
    ...
    for row,p in zip(rows,prov):
        ...
        assert p["parent_record_sha256"]==expected and p["independent_recalculation"] is True and "(upper+lower)*height == 2*area PASS" in p["verification_evidence"]
        assert all(row["audit"][k] is True for k in ("problem_answer_verified","structure_verified","figure_refs_verified"))
    print("PASS_TRAPEZOID_HEIGHT_FROM_AREA_UNIFIED_CONTRACT")
