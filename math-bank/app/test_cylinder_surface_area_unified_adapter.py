...
def main() -> None:
    ...
    parent["id"] = "U-CYL-SA"
    parent["question"] = "半径3cm、高さ5cmの円柱の表面積を、円周率を3.14として求めなさい。"
    parent["answer"] = "150.72cm²"
    parent["choices"] = None
    parent["figure_refs"] = []
    ...
    assert reason.startswith("specialized:cylinder_volume_pi_3_14:cylinder_surface_area_pi_3_14_exact")
    ...
    for row, evidence in zip(rows, prov):
        ...
        assert "cylinder_surface_area_exact_2_pi_r_r_plus_h_and_face_decomposition" in evidence["verification_evidence"]
        assert "PASS" in evidence["verification_evidence"]

    print("PASS_CYLINDER_SURFACE_AREA_UNIFIED_PARENT_SHA_METADATA_AUDIT_AND_RECALCULATION")
