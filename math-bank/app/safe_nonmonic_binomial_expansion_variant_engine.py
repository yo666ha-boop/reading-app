...
def _public_surface(a,b,c,d): return (str(a),str(abs(b)),str(c),str(abs(d)))
def _xcoef(n):
    if n==1: return "+x"
    if n==-1: return "-x"
    return f"{_sgn(n)}x"
...
def generate(parent,count):
    ...
    parent_surface=_public_surface(pa,pb,pc,pd); seen_surfaces=set(); rows=[]; ev=[]
    for i in range(1,count+1):
        ...
        found=False
        for _ in range(33):
            surface=_public_surface(a,b,c,d)
            if surface!=parent_surface and surface not in seen_surfaces and b!=0 and d!=0:
                found=True; break
            d += 1 if d>0 else -1
        if not found: raise AssertionError("nonmonic binomial public numeric-surface search exhausted")
        seen_surfaces.add(surface); A=a*c; B=a*d+b*c; C=b*d
        ...
        rows.append({"question":nq,"answer":ans,"explanation":f"{a*c}x²+({_sgn(a*d)}{_sgn(b*c)})x+({b}×{d})を整理して{ans}。3係数を独立確認済み。","numeric_signature":surface})
