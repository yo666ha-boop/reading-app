...
def _parse(parent):
    ...
    if am is None or (int(am.group("A")),int(am.group("B")),int(am.group("C")))!=(A,B,C): return None
...
def generate(parent,count):
    ...
    for i in range(1,count+1):
        ...
        expr=f"({a}x{_sgn(b)})({c}x{_sgn(d)})"; nq=q[:m.start("expr")]+expr+q[m.end("expr"):]; ans=f"{A}x²{_sgn(B)}x{_sgn(C)}"
