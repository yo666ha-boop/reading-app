...
NOW="2026-08-24T00:58:00Z"

def main():
    parent=copy.deepcopy(make_base()[0]); parent["id"]="U-TRAP-B"
    parent["question"]="上底6cm、高さ8cm、面積64cm²の台形の下底を求めなさい。"; parent["answer"]="10cm"
    ...
    rows,prov,reason=generate_parent(parent,3,NOW); assert reason.startswith("specialized:trapezoid_area:trapezoid_base_from_area_exact")
    ...
    for row,p in zip(rows,prov):
        ...
        assert "missing_base" in p["verification_evidence"] and "PASS" in p["verification_evidence"]
        assert all(row["audit"][k] is True for k in ("problem_answer_verified","structure_verified","figure_refs_verified"))
    print("PASS_TRAPEZOID_BASE_FROM_AREA_UNIFIED_CONTRACT")
