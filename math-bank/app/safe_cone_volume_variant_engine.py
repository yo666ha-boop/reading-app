...
def _parse_parent(parent: dict):
    ...
    if product % 3 != 0:
        return None
    third_product = Decimal(product // 3)
    expected = PI * third_product
    ...
    if actual != expected:
        return None
    if expected / PI != third_product or expected / third_product != PI:
        return None
    if expected * Decimal(3) != PI * Decimal(product):
        return None
...
def generate(parent: dict, count: int) -> tuple[list[dict], list[dict], str]:
    ...
    for index in range(1, count + 1):
        radius, height = _variant_numbers(seed, index)
        sig = (radius, height)
        ...
        product = radius * radius * height
        if product % 3 != 0:
            raise AssertionError("cone volume divisibility invariant failed")
        third_product = Decimal(product // 3)
        value = PI * third_product
        if value / PI != third_product or value / third_product != PI:
            raise AssertionError("cone volume pi identity failed")
        if value * Decimal(3) != PI * Decimal(product):
            raise AssertionError("cone volume one-third identity failed")
        ...
        rows.append({
            ...
            "explanation": f"円すいの体積=円周率×半径×半径×高さ÷3より、3.14×{radius}×{radius}×{height}÷3={_fmt(value)}cm³。3倍して円周率×半径²×高さに戻ることも確認済み。",
            "numeric_signature": (str(radius), str(height), "3.14"),
        })
        evidence.append({
            "parent_sha256": _sha(parent),
            "method": "cone_volume_exact_pi_3_14_one_third_product_and_inverse_identities",
            "parent_recalculation": f"3.14×{parent_radius}×{parent_radius}×{parent_height}÷3={_fmt(parent_value)}cm³",
            "variant_recalculation": f"3.14×{radius}×{radius}×{height}÷3={_fmt(value)}cm³",
            "independent_check": "3V == 3.14*r^2*h AND V/3.14 == r^2*h/3 AND V/(r^2*h/3) == 3.14 PASS",
        })
