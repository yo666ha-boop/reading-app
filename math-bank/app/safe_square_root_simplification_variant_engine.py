...
"""Fail-closed exact engine for simplifying one integer square root.

Accepted parents contain exactly one expression of the form √N, ask only to
simplify it, have no figure/real choices, and have an answer exactly equal to
a√b (or an integer when b=1). Verification uses N=a^2*b and requires b to be
square-free. Generated siblings use different numeric radicands and are checked
...
def _parse(parent: dict):
    ...
    expected = _format(a, b)
    if ans != expected:
        return None
    if a == 1:  # already square-free: not a meaningful simplification parent
        return None
...
def generate(parent: dict, count: int) -> tuple[list[dict], list[dict], str]:
    ...
    while len(rows) < count:
        ...
        rows.append({
            "question": new_q,
            "answer": _format(a, b),
            "explanation": f"{n}={a}²×{b} なので、√{n}={a}√{b}。",
            "numeric_signature": (str(n),),
        })
        evidence.append({
            "parent_sha256": _parent_sha(parent),
            "method": "square_root_exact_factorization_and_squarefree_identity",
            "parent_recalculation": f"{parent_n}={parent_a}^2*{parent_b}",
            "variant_recalculation": f"{n}={a}^2*{b}",
            "independent_check": "coefficient^2 * squarefree_radicand == original AND radicand square-free PASS",
        })
