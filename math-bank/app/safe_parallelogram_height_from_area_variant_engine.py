...
"""Fail-closed exact engine for parallelogram height from base and area."""
...
def _parse(parent):
    ...
    if "平行四辺形" not in q or "高さ" not in q or not any(t in q for t in ("求め","何")):
        return None
    if any(t in q for t in ("底辺を","周","相似","図","グラフ","台形","三角形")):
        return None
    ...
    if base<=0 or area<=0 or area%base: return None
    height=area//base
...
def can_generate(parent):
    if _parse(parent) is not None: return True,"parallelogram_height_from_area_exact"
    if parent.get("figure_refs"): return False,"figure_parent"
    if parent.get("choices"): return False,"choice_parent"
    return False,"parallelogram_height_from_area_parent_not_exactly_parsed_and_verified"
...
def generate(parent,count):
    ...
    for i in range(1,count+1):
        base=3+((seed>>(i*5))+i*5)%14
        height=2+((seed>>(i*7+2))+i*3)%12
        area=base*height; sig=(base,area)
        while sig==(pbase,parea) or sig in seen:
            height+=1; area=base*height; sig=(base,area)
        seen.add(sig)
        if area//base!=height or base*height!=area: raise AssertionError("parallelogram height identity failed")
        repl=f"底辺{base}{unit}、面積{area}{unit}²"; nq=q[:m.start()]+repl+q[m.end():]
        rows.append({"question":nq,"answer":f"{height}{unit}","explanation":f"高さ=面積÷底辺より、{area}÷{base}={height}{unit}。底辺×高さ={area}{unit}²でも確認済み。","numeric_signature":(str(base),str(area))})
        evidence.append({"parent_sha256":_sha(parent),"method":"parallelogram_height_from_area_exact_inverse_and_recomposition","parent_recalculation":f"{parea}/{pbase}={pheight}{unit}","variant_recalculation":f"{area}/{base}={height}{unit}","independent_check":"base*height == area PASS"})
    return rows,evidence,"parallelogram_height_from_area_exact"
