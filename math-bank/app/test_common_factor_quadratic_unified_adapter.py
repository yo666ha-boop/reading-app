...
NOW = "2026-08-24T06:28:00Z"


def main() -> None:
    p = copy.deepcopy(make_base()[0])
    p["id"] = "U-COMMON-FACTOR"
    p["question"] = "6x²+9xを因数分解しなさい。"
    p["answer"] = "3x(2x+3)"
    p["choices"] = []
    p["figure_refs"] = []
    ...
    assert reason.startswith("specialized:monic_quadratic_factorization:common_factor_quadratic_exact"), reason
    ...
    for row, ev in zip(rows, prov):
        ...
        assert "common_factor_gcd_and_exact_reexpansion" in ev["verification_evidence"]
    print("PASS_COMMON_FACTOR_QUADRATIC_UNIFIED_CONTRACT")
