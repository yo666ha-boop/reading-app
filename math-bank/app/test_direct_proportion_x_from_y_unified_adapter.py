...
def parent():
    p=copy.deepcopy(make_base()[0]); p['id']='U-DIR-X'; p['question']='yはxに比例し、比例定数は4です。y=24のとき、xの値を求めなさい。'; p['answer']='x=6'; p['choices']=[]; p['figure_refs']=[]; p['source']['is_generated_variant']=False; p['source']['parent_id']=None; p['variant_group']=None; return p
def main():
    p=parent(); rows,prov,reason=generate_parent(p,3,NOW); assert reason.startswith('specialized:affine:direct_proportion_x_from_y_exact'); assert len(rows)==len(prov)==3
    ...
    for r,e in zip(rows,prov):
        ...
        assert e['parent_record_sha256']==expected and e['independent_recalculation'] is True and 'direct_proportion_x_from_y_exact_division_and_product_identity' in e['verification_evidence']
    bad=parent(); bad['answer']='x=5'; assert generate_parent(bad,1,NOW)[0]==[]
    print('PASS_DIRECT_PROPORTION_X_FROM_Y_UNIFIED_PARENT_SHA_METADATA_AUDIT3_NUMERIC_DISTINCT')
