...
def main() -> None:
    ...
    parent["id"] = "U-CUBE-SIDE-FROM-SURFACE"
    parent["question"] = "表面積が150cm²の立方体があります。この立方体の1辺を求めなさい。"
    parent["answer"] = "5cm"
    parent["choices"] = None
    parent["figure_refs"] = []
    ...
    assert reason.startswith("specialized:cube_volume:cube_surface_area_to_integer_side_exact")
    ...
    for row, evidence in zip(rows, prov):
        ...
        assert "cube_side_from_surface_area_exact_division_square_root_and_recomposition" in evidence["verification_evidence"]
    print("PASS_CUBE_SIDE_FROM_SURFACE_AREA_UNIFIED_PARENT_CONTRACT")
