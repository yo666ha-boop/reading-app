...
REAL_BROWSER_CONTRACT_TEST = ROOT / "test_real_browser_dynamic_contract.py"
...
required_fragments = [
    "original:1124", "baselineVariant:107", "Winpass:570", "'実力錬成':237", "Standard:317",
    "variant>=BASELINE.baselineVariant", "taxonomyFail===0",
    "expandedVariant:Math.max(0,variant-BASELINE.baselineVariant)", "拡張ゲート PASS", "追加${a.expandedVariant}問",
    ".problem-title", ".choices", "validChoices", "choicesHtml", "r?.title",
    "Array.isArray(r?.choices)?r.choices:[]", "Object.prototype.hasOwnProperty.call(r,'choices')", "typeof r.title!=='string'",
    "choiceRecords", "選択問題", ".answer.open", "show-answers-screen", "print-answers", "afterprint", "localStorage",
    "SETTINGS_KEY", "readSettings", "setStoredValue", "restoreSettings", "setStoredValue('major',x.major)",
    "setStoredValue('minor',x.minor)", "sourceAnchor", "const aa=sourceAnchor(a),bb=sourceAnchor(b)", "isVariant(a)?1:0",
    "labelDifficulty", "setOptions($('difficulty'),uniq(base.map(r=>r?.difficulty)),labelDifficulty)", "Math.max(1,Math.min(200",
    "app-records.json", "canonicalAudit", "sourceSort", "parentBookOf", "VALID_GRADES", "VALID_DIFFICULTY", "VALID_BOOKS",
    "SAFE_EXTERNAL_FIGURE_SCHEMES", "SAFE_LOCAL_FIGURE_EXTENSIONS", "safeFigureRef", "figs.some(x=>!safeFigureRef(x))",
    "figurePrintReadiness", "loading=\"eager\"", "dataset.loadError", "naturalWidth===0", "印刷中止：表示中の図版",
    "印刷待機：図版", "auditFail", "parentFail", "taxonomyFail", "figureFail", "problem_answer_verified!==true",
    "structure_verified!==true", "figure_refs_verified!==true", "parentBookOf(r)===book", "以前の表示を破棄しました",
    "render_figure_markers.js", "MikamiMathFigureMarkers", "markerHtml", "markerRefsForRecord", "markerFail",
    "data-inline-figure=\"1\"", "#list img[data-inline-figure=\"1\"]",
]
...
for forbidden in ("const CANONICAL={total:1231", "rs.length===CANONICAL.total", "以外は追加しません", "正本1231問の完全ゲート"):
    if forbidden in text:
        raise SystemExit(f"FAIL frozen-1231 behavior remains: {forbidden}")

for target in (FIGURE_RENDERER, FIGURE_TEST, EXPANDED_VALIDATOR, EXPANDED_COMPOSER, EXPANDED_TEST, EXPANDED_LAYER, REAL_BROWSER_CONTRACT_TEST):
...
proc = subprocess.run(["python", str(REAL_BROWSER_CONTRACT_TEST)], cwd=ROOT, text=True, capture_output=True)
if proc.returncode or "PASS_REAL_BROWSER_DYNAMIC_EXPANDED_CONTRACT" not in proc.stdout:
    raise SystemExit(f"FAIL dynamic real-browser/publication contract regression\n{proc.stdout}\n{proc.stderr}")
...
print("dynamic_real_browser_release_contract=PASS immutable_base_prefix=PASS expanded_parent_coverage=PASS")
