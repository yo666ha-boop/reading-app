...
def main():
    w=[row('Winpass',i) for i in range(717)]; j=[row('実力錬成',i) for i in range(237)]; s=[row('Standard',i) for i in range(317)]; base=w+j+s; frozen,manifest=f.freeze(w,j,s,evidence(base),set())
    with tempfile.TemporaryDirectory() as td:
        ...
        report=m.compose(bp,mp,lp,op,sp); assert report['status']=='PASS' and report['base_records']==1271 and report['baseline_verified_variants']==0 and report['expanded_verified_variants']==0 and report['final_total']==1271; assert json.loads(op.read_text(encoding='utf-8'))==frozen; assert json.loads(sp.read_text(encoding='utf-8'))==frozen
    ...
    print('PASS_REBUILT_DYNAMIC_ANCHOR_APP_COMPOSER_IMMUTABLE_1271_SNAPSHOT')
