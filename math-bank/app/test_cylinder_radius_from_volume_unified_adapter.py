...
def main() -> None:
    ...
    parent["id"] = "U-CYL-R-INV"
    parent["question"] = "高さ5cm、体積141.3cm³の円柱の半径を、円周率を3.14として求めなさい。"
    parent["answer"] = "3cm"
    parent["choices"] = None
    parent["figure_refs"] = []
    ...
    assert reason.startswith("specialized:cylinder_volume_pi_3_14:cylinder_radius_from_volume_pi_3_14_exact")
    ...
    for row, evidence in zip(rows, prov):
        ...
        assert "cylinder_radius_exact_division_square_root_and_volume_recomposition" in evidence["verification_evidence"]
        assert "PASS" in evidence["verification_evidence"]

    print("PASS_CYLINDER_RADIUS_FROM_VOLUME_UNIFIED_PARENT_SHA_METADATA_AUDIT_AND_RECALCULATION")
