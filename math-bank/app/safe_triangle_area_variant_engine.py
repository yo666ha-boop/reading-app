...
"""Fail-closed exact engine for triangle area, inverse-area, and third-angle parents."""

import hashlib
import json
import re

from safe_triangle_base_from_area_variant_engine import generate as generate_base_from_area
from safe_triangle_height_from_area_variant_engine import generate as generate_height_from_area

TRI_RE = re.compile(r"底辺\s*(?P<base>\d+)\s*(?P<unit>mm|cm|m)\s*[、,，]?\s*高さ\s*(?P<height>\d+)\s*(?P=unit)")
...
def _parse_area(parent: dict):
    ...
    if len(matches) != 1:
        return None
    m = matches[0]
    base = int(m.group("base")); height = int(m.group("height")); unit = m.group("unit")
...
def _parse_third_angle(parent: dict):
    if parent.get("figure_refs") or _choice_parent(parent):
        return None
    q = _norm(parent.get("question"))
    if "三角形" not in q or not any(token in q for token in ("残り", "もう1つ", "もう一つ", "3つ目", "三つ目")):
        return None
    ...
    if len(matches) != 1:
        return None
    m = matches[0]
    a = int(m.group("a")); b = int(m.group("b")); c = 180 - a - b
    if min(a, b, c) <= 0:
        return None
    am = ANGLE_ANSWER_RE.fullmatch(_norm(parent.get("answer")).replace(" ", ""))
    if am is None or int(am.group("v")) != c or a + b + c != 180:
        return None
    return m, a, b, c
...
def _delegate_inverse(parent: dict, count: int):
    ...
    return [], [], "triangle_inverse_area_parent_not_exactly_parsed_and_verified"
...
def _generate_area(parent: dict, parsed, count: int):
    ...
    for index in range(1, count + 1):
        base, height = _area_variant_numbers(seed, index); signature=(str(base), str(height)); bump=0
        while signature == parent_signature or signature in seen:
            bump += 1; height += 2 * bump; signature=(str(base), str(height))
        seen.add(signature); product=base*height
        if product % 2:
...
def _generate_third_angle(parent: dict, parsed, count: int):
    ...
    q=_norm(parent.get("question")); seed=int(_parent_sha(parent)[:12],16)
    parent_signature=(str(parent_a),str(parent_b)); seen=set(); rows=[]; evidence=[]
    for index in range(1,count+1):
        a,b=_angle_variant_numbers(seed,index); signature=(str(a),str(b)); bump=0
        while signature==parent_signature or signature in seen:
            bump+=1; a=25+((a+7*bump)%65); b=20+((b+9*bump)%65)
            while a+b>=175: b=20+((b+9)%65)
            signature=(str(a),str(b))
        seen.add(signature); c=180-a-b
        if c<=0 or a+b+c!=180: raise AssertionError("triangle angle-sum identity failed")
        replacement=f"{a}°と{b}°"; new_question=q[:match.start()]+replacement+q[match.end():]
