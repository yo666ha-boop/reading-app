...
def main():
    ...
    parent["id"]="U-RECT-PERIM-SIDE"
    parent["question"]="たて8cm、周の長さ26cmの長方形があります。横の長さを求めなさい。"
    parent["answer"]="5cm"
    parent["choices"]=None
    parent["figure_refs"]=[]
    ...
    assert reason.startswith("specialized:rectangle_perimeter:rectangle_missing_side_from_perimeter_exact")
    ...
    for row,evidence in zip(rows,prov):
        ...
        assert "rectangle_missing_side_from_perimeter_half_and_recomposition" in evidence["verification_evidence"]
    print("PASS_RECTANGLE_SIDE_FROM_PERIMETER_UNIFIED_ADAPTER")
