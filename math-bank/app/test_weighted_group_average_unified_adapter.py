...
NOW="2026-08-23T08:20:00Z"


def main():
    parent=copy.deepcopy(make_base()[0])
    parent["id"]="U-WAVG"
    parent["question"]="A組は20人で平均70点、B組は30人で平均80点です。全体の平均点を求めなさい。"
    parent["answer"]="76"
    parent["choices"]=[]
    parent["figure_refs"]=[]
    ...
    assert reason.startswith("specialized:weighted_group_average:")
    ...
    for row,p in zip(rows,prov):
        ...
        assert "engine=weighted_group_average" in p["verification_evidence"]
        assert "combined_mean*combined_count" in p["verification_evidence"]
    print("PASS_WEIGHTED_GROUP_AVERAGE_UNIFIED_PARENT_SHA_METADATA_AUDIT_AND_IDENTITY")
