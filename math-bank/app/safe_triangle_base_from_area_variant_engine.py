...
"""Fail-closed exact engine for triangle base from height and area."""
...
def _parse(parent):
    ...
    if "三角形" not in q or "底辺" not in q or not any(t in q for t in ("求め","何")):
        return None
    if any(t in q for t in ("高さを","周","相似","図","グラフ","台形","平行四辺形")):
        return None
    ...
    if height<=0 or area<=0 or (2*area)%height: return None
    base=2*area//height
...
def can_generate(parent):
    if _parse(parent) is not None: return True,"triangle_base_from_area_exact"
    if parent.get("figure_refs"): return False,"figure_parent"
    if parent.get("choices"): return False,"choice_parent"
    return False,"triangle_base_from_area_parent_not_exactly_parsed_and_verified"
...
def generate(parent,count):
    ...
    for i in range(1,count+1):
        ...
        base=2+((seed>>(i*7+2))+i*5)%13
        if base*height%2: base+=1
        area=base*height//2; sig=(height,area)
        while sig==(pheight,parea) or sig in seen:
            base+=2; area=base*height//2; sig=(height,area)
        seen.add(sig)
        if 2*area!=base*height or 2*area//height!=base: raise AssertionError("triangle base identity failed")
        repl=f"高さ{height}{unit}、面積{area}{unit}²"; nq=q[:m.start()]+repl+q[m.end():]
        rows.append({"question":nq,"answer":f"{base}{unit}","explanation":f"底辺=2×面積÷高さより、2×{area}÷{height}={base}{unit}。底辺×高さ÷2={area}{unit}²でも確認済み。","numeric_signature":(str(height),str(area))})
        evidence.append({"parent_sha256":_sha(parent),"method":"triangle_base_from_area_exact_inverse_and_recomposition","parent_recalculation":f"2*{parea}/{pheight}={pbase}{unit}","variant_recalculation":f"2*{area}/{height}={base}{unit}","independent_check":"base*height == 2*area PASS"})
    return rows,evidence,"triangle_base_from_area_exact"
