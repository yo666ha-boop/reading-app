...
Parent and variants are verified with exact Fraction arithmetic and an
independent cross-product identity. Figures, real choices, tax/points/profit,
successive discounts, unknown rates, rounding, or ambiguous contexts fail
closed.
"""
...
REVERSE_RE = re.compile(
    r"(?P<expr>(?P<pct>\d+)\s*[%％]\s*(?:引き|値引き)(?:で|して|にして)?\s*(?P<final>\d+)\s*円)"
)
...
def _parse_parent(parent: dict):
    ...
    if reverse_requested:
        ...
        if len(matches) != 1:
            return None
        m = matches[0]
        pct = int(m.group("pct")); final = int(m.group("final"))
        if final <= 0 or pct <= 0 or pct >= 100:
            return None
        base = Fraction(final * 100, 100 - pct)
        if base.denominator != 1 or answer != base.numerator:
            return None
        if Fraction(final * 100, 1) != base * (100 - pct):
            return None
    ...
    if len(matches) != 1:
        return None
    m = matches[0]
    base = int(m.group("base")); pct = int(m.group("pct"))
    if base <= 0 or pct <= 0 or pct >= 100:
        return None
    final = Fraction(base * (100 - pct), 100)
    if final.denominator != 1 or answer != final.numerator:
        return None
...
def can_generate(parent: dict) -> tuple[bool, str]:
    ...
    if parsed is not None:
        return True, "discount_original_price_integer_yen_exact" if parsed[0] == "reverse" else "discount_final_price_integer_yen_exact"
...
def generate(parent: dict, count: int) -> tuple[list[dict], list[dict], str]:
    ...
    seen: set[tuple[str, str]] = set(); rows: list[dict] = []; evidence: list[dict] = []
    ...
    for index in range(1, count + 1):
        ...
        while True:
            ...
            bump += 1
            if bump > 32:
                raise AssertionError("discount bounded distinctness search exhausted")
            base += 100 * bump
        seen.add(signature)
        if final * 100 != base * (100 - pct):
        ...
        if mode == "reverse":
            replacement = f"{pct}%引きで{final.numerator}円"
            new_question = q[:match.start("expr")] + replacement + q[match.end("expr"):]
            answer = f"{base}円"
            explanation = f"{pct}%引き後は元の価格の{100-pct}%です。{final.numerator}×100/{100-pct}={base}円。割引後価格の再計算でも確認済み。"
            method = "discount_original_price_exact_fraction_and_cross_multiply"
            parent_recalc = f"{parent_final}×100/{100-parent_pct}={parent_base}円"
            variant_recalc = f"{final.numerator}×100/{100-pct}={base}円"
            independent = "list_price*(100-discount_percent) == final*100 PASS"
            reason = "discount_original_price_integer_yen_exact"
        else:
            ...
            parent_recalc = f"{parent_base}×{100-parent_pct}/100={parent_final}円"
            variant_recalc = f"{base}×{100-pct}/100={final.numerator}円"
            independent = "final*100 == list_price*(100-discount_percent) PASS"
            reason = "discount_final_price_integer_yen_exact"
