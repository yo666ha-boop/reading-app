...
def main():
    ...
    parent["id"]="U-RPOLY-INTERIOR-INVERSE"
    parent["question"]="1つの内角が135°である正多角形は何角形ですか。"
    parent["answer"]="8角形"
    parent["choices"]=None
    parent["figure_refs"]=[]
    ...
    assert "regular_polygon_sides_from_single_interior_angle_exact" in reason, reason
    ...
    for row,evidence in zip(rows,prov):
        ...
        assert "engine=regular_polygon_interior_angle" in evidence["verification_evidence"], evidence["verification_evidence"]
        assert row["answer"].endswith("角形")
    print("PASS_REGULAR_POLYGON_SIDES_FROM_INTERIOR_UNIFIED_PARENT_CONTRACT",reason)
