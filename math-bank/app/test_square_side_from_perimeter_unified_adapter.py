...
def main():
    ...
    parent["id"]="U-SQ-SIDE-P"
    parent["question"]="周の長さ32cmの正方形の1辺の長さを求めなさい。"
    parent["answer"]="8cm"
    parent["choices"]=None
    parent["figure_refs"]=[]
    ...
    assert reason.startswith("specialized:square_area:square_side_from_perimeter_exact_division_by_four")
    ...
    for row,evidence in zip(rows,prov):
        ...
        assert "square_side_from_perimeter_exact_division_and_recomposition" in evidence["verification_evidence"]
    print("PASS_SQUARE_SIDE_FROM_PERIMETER_UNIFIED_ADAPTER")
