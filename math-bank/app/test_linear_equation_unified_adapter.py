...
def main():
    p=copy.deepcopy(make_base()[0]); p["id"]="U-LIN-EQ"; p["question"]="方程式3x+2=20を解きなさい。"; p["answer"]="x=6"; p["choices"]=None; p["figure_refs"]=[]; p["source"]["is_generated_variant"]=False; p["source"]["parent_id"]=None; p["variant_group"]=None
    rows,prov,reason=generate_parent(p,3,NOW); assert reason.startswith("specialized:affine:linear_equation_ax_plus_b_equals_c_exact"); assert len(rows)==len(prov)==3
    ...
    for row,ev in zip(rows,prov):
        ...
        assert "linear_equation_exact_inverse_and_forward_recomposition" in ev["verification_evidence"]
    print("PASS_LINEAR_EQUATION_UNIFIED_ADAPTER")
