...
ANSWER_RE = re.compile(r"^(?P<freq>\d+)\s*(?:人|個|回|名)?$")
...
def _parse_parent(parent:dict):
    ...
    if "相対度数" not in q or not any(t in q for t in ("度数を求","度数は何","何人","何個","何回","何名")):
        return None
...
def _fmt(frac:Fraction)->str:
    ...
    s=format(d,"f"); return s.rstrip("0").rstrip(".") if "." in s else s
...
def generate(parent:dict,count:int)->tuple[list[dict],list[dict],str]:
    ...
    if parsed is None:
        ...
        return [],[],reason
    match,ptotal,prel,pfreq=parsed; q=_norm(parent.get("question")); seed=int(_sha(parent)[:12],16)
    ...
    for index in range(count):
        ...
        freq_i=freq.numerator; sig=(str(total),_fmt(rel))
        bump=0
        ...
        rows.append({"question":new_q,"answer":str(freq_i),"explanation":f"度数=全体の度数×相対度数より、{total}×{_fmt(rel)}={freq_i}。{freq_i}/{total}={_fmt(rel)}でも確認済み。","numeric_signature":sig})
