...
"""Fail-closed exact engine for text-only cone surface-area parents using pi=3.14."""
...
def _parse_parent(parent: dict):
    ...
    if not any(token in q for token in ("円すい", "円錐")) or "表面積" not in q or "円周率" not in q or "3.14" not in q:
        return None
    ...
    slant_matches = list(SLANT_RE.finditer(q))
    if len(radius_matches) != 1 or len(slant_matches) != 1:
        return None
    rm, lm = radius_matches[0], slant_matches[0]
    radius, slant = int(rm.group("radius")), int(lm.group("slant"))
    if radius <= 0 or slant <= 0 or slant < radius:
        return None
    ...
    if base + lateral != expected:
        return None
    if expected / (PI * Decimal(radius)) != Decimal(radius + slant):
        return None
...
def generate(parent: dict, count: int) -> tuple[list[dict], list[dict], str]:
    ...
    if parsed is None:
        ...
        assert not ok
        return [], [], reason
    rm, lm, parent_radius, parent_slant, parent_value = parsed
    q = _norm(parent.get("question"))
    ...
    for index in range(1, count + 1):
        radius = 2 + ((seed >> (index * 5)) + index * 5) % 10
        slant = radius + 2 + ((seed >> (index * 7 + 3)) + index * 7) % 12
        sig = (radius, slant)
        while sig == (parent_radius, parent_slant) or sig in seen:
            slant += 1
            sig = (radius, slant)
        seen.add(sig)
        value = PI * Decimal(radius) * Decimal(radius + slant)
        base = PI * Decimal(radius * radius)
        lateral = PI * Decimal(radius * slant)
        if base + lateral != value:
            raise AssertionError("cone surface area face decomposition failed")
        if value / (PI * Decimal(radius)) != Decimal(radius + slant):
            raise AssertionError("cone surface area inverse identity failed")
        replacements = [(rm.start(), rm.end(), f"半径{radius}cm"), (lm.start(), lm.end(), f"母線{slant}cm")]
        new_q = q
        ...
        rows.append({
            ...
            "explanation": f"円すいの表面積=底面積+側面積=3.14×{radius}×{radius}+3.14×{radius}×{slant}={_fmt(value)}cm²。3.14×半径×(半径+母線)でも確認済み。",
            "numeric_signature": (str(radius), str(slant), "3.14"),
        })
        evidence.append({
            "parent_sha256": _sha(parent),
            "method": "cone_surface_area_exact_pi_r2_plus_pi_r_l_and_factored_identity",
            "parent_recalculation": f"3.14×{parent_radius}×{parent_radius}+3.14×{parent_radius}×{parent_slant}={_fmt(parent_value)}cm²",
            "variant_recalculation": f"3.14×{radius}×{radius}+3.14×{radius}×{slant}={_fmt(value)}cm²",
            "independent_check": "pi*r^2 + pi*r*l == S AND S/(pi*r) == r+l PASS",
        })
