...
EXPECTED_TOTAL = 1271
EXPECTED_COUNTS = {"Winpass":717,"実力錬成":237,"Standard":317}
...
def validate(records:list[dict], evidence:list[dict])->dict:
    ...
    if len(records)!=EXPECTED_TOTAL: issues.append({"issue":"record_count_mismatch","expected":EXPECTED_TOTAL,"actual":len(records)})
    for i,r in enumerate(records):
        try: _app.validate_record(r,seen)
        except Exception as exc:
            issues.append({"index":i,"id":r.get("id"),"issue":"app_schema_validation_failed","detail":str(exc)}); continue
        rid=str(r["id"]).strip(); by_id[rid]=r
        src=r.get("source",{})
        if src.get("is_generated_variant") is not False: issues.append({"index":i,"id":rid,"issue":"mapped_parent_must_be_original"})
        if src.get("book") not in EXPECTED_COUNTS: issues.append({"index":i,"id":rid,"issue":"mapped_parent_invalid_source_book","actual":src.get("book")})
        else: source_counts[src["book"]]+=1
    ...
    if duplicates: issues.append({"issue":"duplicate_mapping_evidence_ids","ids":duplicates[:20]})
    ...
    required=("record_id","record_sha256","source_document","source_locator","grade_evidence","unit_evidence","skill_evidence","question_format_evidence","difficulty_evidence")
    for rid in sorted(set(by_id)&set(evidence_map)):
        ...
        if str(e.get("record_sha256","")).strip().lower()!=canonical_sha(r): errs.append("record_sha256 mismatch")
    ...
    if dict(source_counts)!=EXPECTED_COUNTS: issues.append({"issue":"mapped_source_counts_mismatch","expected":EXPECTED_COUNTS,"actual":dict(source_counts)})
    return {"records":len(records),"mapping_evidence":len(evidence),"source_counts":dict(source_counts),"issues":issues,"pass":not issues,"policy":{"all_1271_app_records_must_validate":True,"every_parent_requires_exact_record_bound_mapping_evidence":True,"mapping_defaults_without_source_evidence_forbidden":True,"source_locator_required":True,"grade_unit_skill_question_format_difficulty_evidence_required":True,"only_pass_may_feed_variant_generation":True}}
