...
class StrictCandidateComposerTest(unittest.TestCase):
    ...
    def test_missing_score_evidence_blocks_candidate(self):
        candidates, report = composer.compose([draft()], locator_report(locator()), [], [])
        self.assertEqual(candidates, [])
        self.assertEqual(report["unresolved"], 1)
        self.assertTrue(any("score_evidence" in reason for reason in report["details"][0]["reasons"]))
    ...
    def test_figure_asset_identity_is_carried_into_fingerprint(self):
        candidates, report = composer.compose([draft()], locator_report(locator(figure=True)), [slot()], [])
        self.assertEqual(report["strict_candidates"], 1)
        record = candidates[0]
    ...
    def test_graphical_answer_requires_source_bound_asset(self):
        ...
        candidates, report = composer.compose([graphical_draft], locator_report(loc), [slot()], evidence)
        self.assertEqual(report["strict_candidates"], 1)
        self.assertEqual(gate.validate_record(candidates[0], 1), [])
        ...
        candidates2, report2 = composer.compose([graphical_draft], locator_report(loc), [slot()], bad)
