...
def evidence(records:list[dict])->list[dict]:
    ...
    return out
def main() -> None:
    w=rows("Winpass",717); j=rows("実力錬成",237); s=rows("Standard",317); all_rows=w+j+s; ev=evidence(all_rows)
    combined,manifest=m.freeze(w,j,s,ev,set())
    assert len(combined)==1271 and manifest["records"]==1271
    assert combined[0]["id"]=="Winpass-0" and combined[716]["id"]=="Winpass-716"
    assert combined[717]["id"]=="実力錬成-0" and combined[953]["id"]=="実力錬成-236"
    assert combined[954]["id"]=="Standard-0" and combined[-1]["id"]=="Standard-316"
    assert len(manifest["provenance"])==1271 and len(manifest["combined_payload_sha256"])==64
    ...
    print("PASS_IMMUTABLE_REBUILT_APP_ORIGINALS_1271_MAPPING_BEFORE_FREEZE_SHA_ORDER_GATES")
