...
def main():
    w=rows("Winpass",717); j=rows("実力錬成",237); s=rows("Standard",317); all_rows=w+j+s
    frozen,manifest=m._f.freeze(w,j,s,evidence(all_rows),set()); ok=m.validate(frozen,manifest); assert ok["pass"] is True and len(frozen)==1271
    ...
    badp=dict(manifest); badp["provenance"]=[dict(x) for x in manifest["provenance"]]; badp["provenance"][717]["source"]="Winpass"; report=m.validate(frozen,badp); assert report["pass"] is False; assert any(x.get("issue")=="source_order_mismatch" for x in report["issues"])
    print("PASS_FROZEN_REBUILT_APP_ORIGINALS_1271_FULL_SHA_EXACT_MANIFEST_PARENT_BINDING_GATES")
