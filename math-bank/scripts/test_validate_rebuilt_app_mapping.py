...
def main()->None:
    records=[rec("Winpass",i,1+(i%3)) for i in range(717)]+[rec("実力錬成",i,1+(i%3)) for i in range(237)]+[rec("Standard",i,1+(i%3)) for i in range(317)]
    evidence=[ev(r) for r in records]
    ok=m.validate(records,evidence); assert ok["pass"] is True and ok["records"]==1271
    ...
    print("PASS_REBUILT_APP_MAPPING_1271_SCHEMA_EXACT_RECORD_EVIDENCE_GATES")
