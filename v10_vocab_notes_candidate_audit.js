...
function tokenize(text){return (String(text||'').replace(/[’]/g,"'").match(/[A-Za-z]+(?:'[A-Za-z]+)*/g)||[]).map(x=>x.toLowerCase());}
...
// Closed, explicit structural allowance only. This is deliberately finite and is NOT a general dictionary.
...
function canonicalCutoff(v7,textbook,grade,section){
  ...
  if(code==='SS'){
    ...
    m=String(section).match(/^PROGRAM\s*(\d+)-(\d+)$/i);
    if(m){const n=Number(m[1]),k=Number(m[2]);let r=k;
      // v7 combines these printed subunits; use the canonical combined boundary, never a later file.
      if(g===1&&n===9&&k>=2)r=Math.max(r,3);
      if(g===1&&n===10&&k===1)r=2;
      if(g===1&&n===10&&k>=4)r=8;
      if(g===2&&n===7&&k===1)r=2;
      if(g===2&&n===8&&k>=2)r=3;
    ...
    m=String(section).match(/^Unit\s*(\d+)-(\d+)$/i);
    if(m){const n=Number(m[1]),k=Number(m[2]);let r;
      if(g===1){r=k<=2?k:(n<=4?3:6);if(n===8&&k===1)r=2;}
      else{r=k===1?1:k===2?2:k===3?4:5;if(g===2&&n===3&&k===1)r=2;if(g===2&&n===7&&k===1)r=2;if(g===3&&n===1&&k===1)r=2;}
      return at(`U${n}`,r);
    }
  }
...
function classifyToken(v7,w,raw,cut){
  const direct=introFor(v7,cut.code,w);
  if(direct&&introAllowed(direct,cut))return {kind:'V7_CHRONOLOGY_ALLOWED',intro:direct};
  const bases=uniq([IRREGULAR_BASE.get(w),...morphologyBases(w)].filter(Boolean));
  for(const base of bases){const bi=introFor(v7,cut.code,base);if(bi&&introAllowed(bi,cut))return {kind:'MORPHOLOGY_TO_GRAMMAR',base,intro:bi};}
  if(CONTRACTIONS_TO_GRAMMAR.has(w))return {kind:'CONTRACTION_TO_GRAMMAR'};
  // Only after canonical lookup fails/comes later do we consult the documented closed structural set.
  if(!direct&&EXPLICIT_FUNCTION_TO_GRAMMAR.has(w))return {kind:'EXPLICIT_FUNCTION_TO_GRAMMAR'};
  if(direct)return {kind:'FUTURE_V7_LEAK',intro:direct};
...
(async()=>{let dom;try{
  ...
  const DATASETS=w.eval('DATASETS'),META=w.eval('META');let total=0,notesPresent=0,missingGloss=0;const passages=[],global=new Map(),mappingErrors=[];
  const counts={V7_CHRONOLOGY_ALLOWED:0,MORPHOLOGY_TO_GRAMMAR:0,CONTRACTION_TO_GRAMMAR:0,EXPLICIT_FUNCTION_TO_GRAMMAR:0,FUTURE_V7_LEAK:0,UNREGISTERED_V7:0,UNREGISTERED_PROPER:0};
  for(const grade of ['1','2','3'])for(const textbook of ['サンシャイン','ニューホライズン'])for(const [section,m] of Object.entries((DATASETS[grade]||{})[textbook]||{})){
    total++;let cut;try{cut=canonicalCutoff(v7,textbook,grade,section);}catch(e){mappingErrors.push(String(e.message||e));continue;}
    ...
    for(const src of sourceStrings(m,meta)){
      ...
      for(const raw of rawTokens){const tok=raw.toLowerCase(),c=classifyToken(v7,tok,raw,cut);counts[c.kind]++;
        if(c.kind==='V7_CHRONOLOGY_ALLOWED')continue;
        const key=`${c.kind}|${tok}|${c.base||''}`;if(!found.has(key))found.set(key,{token:tok,kind:c.kind,base:c.base||null,intro:c.intro||null,locations:[]});
        if(found.get(key).locations.length<10)found.get(key).locations.push(src.where);
        if(['FUTURE_V7_LEAK','UNREGISTERED_V7','UNREGISTERED_PROPER'].includes(c.kind)){
          const gkey=`${c.kind}|${tok}`;if(!global.has(gkey))global.set(gkey,{token:tok,kind:c.kind,intro:c.intro||null,occurrences:0,sections:new Set(),examples:[]});const x=global.get(gkey);x.occurrences++;x.sections.add(`${textbook}|${grade}|${section}`);if(x.examples.length<12)x.examples.push(`${textbook}|${grade}|${section}|${src.where}`);
        }
      }
    }
    const notes=Array.isArray(m.notes)?m.notes:[];notesPresent+=notes.length;for(const n of notes)if(!n||!String(n.english||'').trim()||!String(n.japanese||'').trim())missingGloss++;
    passages.push({grade,textbook,section,id:m.id||'',cutoff:cut,notes:notes.length,candidates:[...found.values()]});
  }
  ...
  const unresolved=[...global.values()].map(x=>({...x,sectionCount:x.sections.size,sections:[...x.sections]})).sort((a,b)=>b.occurrences-a.occurrences||a.token.localeCompare(b.token));
  const finalVocabLeak=counts.FUTURE_V7_LEAK+counts.UNREGISTERED_V7+counts.UNREGISTERED_PROPER;
  const summary={generatedAt:new Date().toISOString(),passages:total,canonicalSource:{spreadsheetId:v7.source[0],sheet:v7.source[1],records:v7.source[2],modifiedTime:v7.source[3]},counts,notesPresent,missingGloss,mappingErrors,uniqueUnresolved:unresolved.length,futureVocabLeakOccurrences:finalVocabLeak,chronologyPass:finalVocabLeak===0&&missingGloss===0&&mappingErrors.length===0,rule:'Primary lexical gate is canonical v7 earliest occurrence by textbook + grade + PDF order + canonical subunit rank. Previous grades are cumulative. Later PDF/subunits are forbidden. Morphology and closed structural forms are never lexical PASS; they are handed to grammar chronology.'};
  ...
  console.log(`unique_unresolved=${unresolved.length} future_vocab_leak_occurrences=${finalVocabLeak} notes=${notesPresent} missing_gloss=${missingGloss} chronologyPass=${summary.chronologyPass}`);
  console.log('REPORT=v10_vocab_notes_candidate_report.json');
}finally{if(dom)dom.window.close();}})().catch(e=>{console.error(`V7 VOCAB CHRONOLOGY AUDIT FAIL: ${e.stack||e}`);process.exit(1)});
