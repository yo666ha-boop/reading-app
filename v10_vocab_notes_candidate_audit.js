...
const IRREGULAR_BASE = new Map(Object.entries({
  went:'go', gone:'go', came:'come', seen:'see', saw:'see', made:'make', took:'take', taken:'take',
  gave:'give', given:'give', wrote:'write', written:'write', read:'read', bought:'buy', brought:'bring',
...
function sourceStrings(m, meta) {
  ...
  const addQuestion = (q, prefix) => {
    for (const k of ['prompt', 'answer', 'evidence']) out.push({ where: `${prefix}.${k}`, text: q && q[k] });
    for (const k of ['choice', 'choices', 'option', 'options']) {
      const v = q && q[k];
      if (Array.isArray(v)) v.forEach((x, j) => out.push({ where: `${prefix}.${k}:${j + 1}`, text: typeof x === 'string' ? x : (x && (x.text || x.label || x.value)) }));
      else if (typeof v === 'string') out.push({ where: `${prefix}.${k}`, text: v });
    }
  };
  (m.questions || []).forEach((q, i) => addQuestion(q, `A${i + 1}`));
  const b = meta && Array.isArray(meta.questionSetB) ? meta.questionSetB : [];
  b.forEach((q, i) => addQuestion(q, `B${i + 1}`));
  return out;
}
function canonicalCode(textbook) {
  if (textbook === 'ニューホライズン') return 'NH';
  if (textbook === 'サンシャイン') return 'SS';
  throw new Error(`unknown textbook ${textbook}`);
}
function canonicalCutoff(v7, textbook, grade, section) {
  const code = canonicalCode(textbook), g = Number(grade), units = v7.units[`${code}|${g}`] || {};
  const at = (fileUnit, subrank) => {
    const pdf = Number(units[fileUnit]);
    if (!pdf) throw new Error(`v7 unit mapping missing ${code}|${g}|${section} -> ${fileUnit}`);
    return { code, grade: g, fileUnit, pdf, subrank: Number(subrank) };
  };
  if (code === 'SS') {
    let m = String(section).match(/^Get Ready\s*(\d+)$/i);
    if (m) return at('Pr', Number(m[1]));
    if (/Step\s*6|Our Project\s*3|Power-Up\s*6/i.test(section)) return at('P10', 8);
    m = String(section).match(/^PROGRAM\s*(\d+)-(\d+)$/i);
    if (m) {
      const n = Number(m[1]), k = Number(m[2]); let r = k;
    ...
    let m = String(section).match(/^Unit\s*0$/i);
    if (m) return at('U0', 9);
    m = String(section).match(/^Unit\s*(\d+)-(\d+)$/i);
    if (m) {
      const n = Number(m[1]), k = Number(m[2]); let r;
      if (g === 1) {
        r = k <= 2 ? k : (n <= 4 ? 3 : 6);
        if (n === 8 && k === 1) r = 2;
      } else {
        r = k === 1 ? 1 : k === 2 ? 2 : k === 3 ? 4 : 5;
        if (g === 2 && n === 3 && k === 1) r = 2;
        if (g === 2 && n === 7 && k === 1) r = 2;
        if (g === 3 && n === 1 && k === 1) r = 2;
      }
...
function introAllowed(intro, cut) {
  if (!intro) return false;
  const [g, pdf, sub] = intro;
  if (Number(g) < cut.grade) return true;
...
function datasetCount(d) {
  let n = 0;
  for (const g of Object.values(d || {})) for (const t of Object.values(g || {})) n += Object.keys(t || {}).length;
  return n;
}
...
(async () => {
  ...
  try {
    ...
    await waitFor(() => {
      const expected = (w.V10_VOCAB_CORRECTIONS || []).length;
    ...
    for (const textbook of ['サンシャイン', 'ニューホライズン']) {
      ...
      for (const grade of ['1', '2', '3']) {
        const entries = Object.entries((DATASETS[grade] || {})[textbook] || {})
          .map(([section, m], index) => {
            try { return { section, m, index, cut:canonicalCutoff(v7, textbook, grade, section) }; }
            catch (e) { mappingErrors.push(String(e.message || e)); return null; }
          })
          .filter(Boolean)
          .sort((a, b) => a.cut.pdf - b.cut.pdf || a.cut.subrank - b.cut.subrank || a.index - b.index);
          ...
          for (const src of sourceStrings(m, meta)) {
            ...
            for (const raw of rawTokens) {
              ...
              if (['FUTURE_V7_LEAK','UNREGISTERED_V7','UNREGISTERED_PROPER'].includes(c.kind)) {
                const gkey = `${c.kind}|${tok}`;
                if (!global.has(gkey)) global.set(gkey, {token:tok,kind:c.kind,intro:c.intro||null,occurrences:0,sections:new Set(),examples:[]});
                const x = global.get(gkey);
                x.occurrences++;
                x.sections.add(`${textbook}|${grade}|${section}`);
                if (x.examples.length < 12) x.examples.push(`${textbook}|${grade}|${section}|${src.where}`);
              }
            }
          }
    ...
    assert(mappingErrors.length === 0, `canonical cutoff mapping errors: ${mappingErrors.join(' | ')}`);
    const unresolved = [...global.values()]
      .map(x => ({...x, sectionCount:x.sections.size, sections:[...x.sections]}))
    ...
    console.log(`unique_unresolved=${unresolved.length} future_vocab_leak_occurrences=${finalVocabLeak} notes=${notesPresent} missing_gloss=${missingGloss} runtime_browser_errors=${browserErrors.length} chronologyPass=${summary.chronologyPass}`);
    console.log('REPORT=v10_vocab_notes_candidate_report.json');
  } finally {
    if (dom) dom.window.close();
  }
})().catch(e => {
  console.log(`V7 VOCAB CHRONOLOGY AUDIT FAIL: ${e.stack || e}`);
  process.exit(1);
});
