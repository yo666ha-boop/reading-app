...
const loader=`function loadDraft(){const build=require('./v11_batch13_build_body_candidate.js');const x=build();if(!x||!Array.isArray(x.passages)||x.passages.length!==50)throw Error('Batch13 candidate not 50');const ps=x.passages.map(p=>{const q=JSON.parse(JSON.stringify(p));const a=q.anchor||{};q.textbook=a.textbook==='Sunshine'?'サンシャイン':a.textbook==='New Horizon'?'ニューホライズン':a.textbook;q.grade=Number(a.grade);q.section=a.unit;q.semanticRewrite=q.humanSemanticReview||'B13_HUMAN_SEMANTIC_COMPLETE';q.notes=q.notes||[];return q;});return ps;}`;
