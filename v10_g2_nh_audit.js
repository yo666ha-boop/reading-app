...
const data=load(['v10_data_newhorizon_g2_unit0_1.js','v10_data_newhorizon_g2_unit0_1_fix.js','v10_data_newhorizon_g2_unit0_1_fix2.js'],'V10_PASSAGES_G2_NH');
const meta=load(['v10_interaction_metadata_nh_g2_u0_u1.js'],'V10_INTERACTION_META_G2_NH_U01');
const expected=['Unit 0','Unit 1-1','Unit 1-2','Unit 1-3','Unit 1-4'];
...
for(const [section,m] of Object.entries(data)){
 const tag=`NH2/${section}`;
 if(m.grade!=='2'||m.textbook!=='ニューホライズン'||m.section!==section)errors.push(`${tag}: identity mismatch`);
 ...
 const md=meta[`ニューホライズン|2|${section}`];if(!md){errors.push(`${tag}: interaction metadata missing`);continue}
...
const future={
 'Unit 0':['be going to','flight','airport','seafood','reservation','excited','language','communicate','chinese','dollar','weigh','culture','painting','surprised','india','mosque'],
 'Unit 1-1':['seafood','reservation','excited','different','language','communicate','chinese','dollar','weigh','culture','painting','surprised','india','mosque'],
 'Unit 1-2':['found','communicate','chinese','different','meter','dollar','weigh','culture','painting','surprised','india','mosque'],
 'Unit 1-3':['culture','painting','surprised','india','mosque']
};
for(const [section,terms] of Object.entries(future)){const t=text(section);for(const term of terms)if(t.includes(term))errors.push(`NH2/${section}: future-vocab leak ${term}`)}
const forbidden=['visited','showed','learned','gave','tried','used','pictures','languages','tourists','victims','badges','materials'];
...
console.log('G2 NH CANONICAL v7=Unit0 + Unit1 Part1/Part2/ReadThink1/ReadThink2; v9 labels=Unit0 + Unit1-1..1-4');
if(errors.length){console.error(`G2 NH AUDIT FAIL ${errors.length}`);for(const e of errors)console.error(`- ${e}`);process.exit(1)}
console.log('G2 NH AUDIT PASS: Unit 0 and Unit 1 passages, slash rows, A/B questions, evidence links, genres, release flags, morphology guard, and chronological vocab gates are consistent.');